...
> **硬原则：宿主是单文件——5 个内置扩展进程内化运行（不再内嵌 exe）。**
...
| 一键出包 | `bash tools/package.sh`（使用 `+stable-x86_64-pc-windows-gnu` 工具链：先 build `dd-ext` → build `dd-gui --bin dd-run` → 单文件产物与 sidecar 归集到 `dist/`；裸 `cargo` 在 MSVC 下会找不到 `std`） |
...
| 验证口径 | `cargo fmt --check` / `cargo clippy --workspace --all-targets` / `cargo test --workspace` **全绿** + 脱离源码树冒烟（内置进程内化就绪 + sidecar 被发现 → warm 握手） |

**为什么不做安装器 / 文件搜索为何保持 sidecar**：项目 MVP 阶段显式选择"简单可实现"——免安装绿色 zip 满足"零安装仪式"，且不必为注册表 / 安装卸载 / 升级脚本付复杂度（M7 批次 7.3 曾探索 Inno Setup 安装器，后按决策放弃，不留安装器代码）。文件搜索不纳入内置打包集：它依赖用户环境依赖（Everything），sidecar 让「带不带文件搜索」与宿主本体解耦（M7 批次 7.4 取舍记档，见 [`docs/implementation.md`](./docs/implementation.md) §5）。
...
> **注（M9）**：内置扩展改为进程内化运行（宿主直接调 `dd_ext::serve_line`），但仍走 NDJSON JSON-RPC 协议；仅文件搜索 sidecar 保留子进程隔离（ADR-1）。
...
- **候选方向**：A2 冷启动 GUI 瓶颈（异步字体修复前 wgpu + 22MB 字体 ~2.8s）、跨平台。完整进度与遗留项台账见 [`docs/implementation.md`](./docs/implementation.md) §5 / §6.1。

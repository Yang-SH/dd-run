...
const I18N: &[(&str, &str, &str)] = &[
    ...
    ("set.backdrop.name", "窗口材质与边框", "Window material & border"),
    (
        "set.backdrop.desc",
        "窗口材质、圆角与面板描边，均即时生效",
        "Window material, corner shape and panel border, applied instantly",
    ),
    // ── 窗口材质与边框卡四行（2026-09-13，参考 DeskBox 外观设置）──
    ("set.material.name", "材质", "Material"),
    (
        "set.material.desc",
        "Windows 11 系统材质，即时生效",
        "Windows 11 system materials, applied instantly",
    ),
    (
        "set.material.none.desc",
        "不透明面板；材质 API 不可用时的回退观感",
        "Opaque panel; the fallback look when system materials are unavailable",
    ),
    (
        "set.material.mica.desc",
        "云母：随桌面壁纸柔化采色，性能开销低",
        "Mica: soft color sampling from the desktop wallpaper, low overhead",
    ),
    (
    ...
    (
        "set.corner.desc",
        "窗口四角的圆角偏好，与材质选择无关",
        "Corner preference for the window, independent of the material",
    ),
    ("set.corner.round", "圆角", "Round"),
    ("set.corner.small", "小圆角", "Small"),
    ("set.corner.square", "方角", "Square"),
    ("set.border.name", "面板边框", "Panel border"),
    (
        "set.border.desc",
        "材质生效时绘制的 1px 描边；「强调色」跟随系统强调色",
        "A 1px outline drawn while a material is active; \"Accent\" follows the system accent",
    ),

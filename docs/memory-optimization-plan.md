...
|---|---|---|---|
| M0 — 四态基线测量 | ✅ | — | 临时探针，跑完已删 |
| M1 — 隐藏后工作集修剪 | ✅ | `crates/dd-gui/src/app/mod.rs`（约 :617-622）+ `platform.rs`（约 :16-21） | 用 `SetProcessWorkingSetSizeEx`，非 `lifecycle.rs::hide()` |
| M2 — 字体栈 mmap 化 | ✅ | `crates/dd-gui/src/platform.rs`（约 :94-118） | `memmap2` + `Box::leak` + `from_static` |
| M3 — 图标纹理缓存 hide 清空 | ✅ | `crates/dd-gui/src/app/mod.rs`（约 :619） | `icon_cache.clear()` |
| M4 — nucleo Matcher 复用 | ❌ | — | 已判定不做（稳态私有无贡献） |
| §7 实测数据 | ✅ | — | 已追加（debug 口径真机采样） |
| 真机复验（长会话数字稳定） | ⚠️ | — | 待做 |
...
| M1 | 隐藏后工作集修剪（`SetProcessWorkingSetSizeEx`） | Task Manager 可见大头：隐藏态 60–100MB → ~15–25MB | ~25 行 | 极低 |
...
- 落点：`crates/dd-gui/src/app/mod.rs` 隐藏帧收尾（约 :617-622，在 `icon_cache.clear()` 之后调用 `crate::platform::trim_working_set()`）；热键 / Esc / 托盘 / 失焦的隐藏全路径经此收口（非旧文档所写的 `lifecycle.rs::hide()`）。
- API：`SetProcessWorkingSetSizeEx(GetCurrentProcess(), usize::MAX, usize::MAX, 0)`（等价旧 API `SetProcessWorkingSetSize(-1,-1)` 的 EmptyWorkingSet 语义；见 `crates/dd-gui/src/platform.rs` 约 :16-21；windows-sys 补 `Win32_System_Memory` feature；非 Windows 空实现）。
- 语义（诚实记档）：OS 常规提示——页移出工作集、按需软故障回（µs 级），**私有提交不变**；收益是后台物理内存占用与 Task Manager 数字的真实下降。
- 与既有机制无冲突：挂在隐藏期既有 tick 内，不新增唤醒源；唤醒首帧软故障不可感。

### 4.3 M2 — 字体栈 mmap 化（唯一动私有提交的措施）
...
- 落法：`load_cjk_font_definitions` 各 `std::fs::read` 改 `memmap2::Mmap`（新增 1 个成熟小依赖）+ `Box::leak` 成 `'static` 切片 + `FontData::from_static`；后台线程加载时序、热替换调用点零改动；仅 Windows 字体路径。落地位置见 `platform.rs` 约 :94-118（`memmap2::Mmap::map` 约 :100、`Box::leak` 约 :103、`from_static` 约 :105）。
- 生命周期：字体 mmap 常驻进程全程（`Box::leak` 有意为之）；Windows Fonts 目录下文件被系统更新替换属极小概率且 mmap 只读不受影响。

### 4.4 M3 — 图标纹理缓存 hide 清空

- `hide()` 中 `icon_cache.clear()`（`app/mod.rs` 约 :619；全部 TextureHandle drop → egui 释放纹理）；`icon_failed` 负缓存保留（防失败重试刷屏的原语义不变）。
- 重现代价：下次唤起仅对可见行（≤13 个）读盘 PNG + 解码 + 上传，合计 <10ms 且惰性；apps 图标抽取落盘缓存原样复用，不重新抽取。

### 4.5 M4 — nucleo Matcher 复用（测量驱动，已判定不做）
...
## 8. 实施结果（2026-09-12，debug 口径真机采样）

...
> **状态**：🟨 代码完成待验收（**逻辑层 + UI 接线（P5/P6）全部落地**：页面栈 +
> 8 种 Kind 裁决 + Confirm 挂起重发 + invoke 参数 + Enter 分派 / Esc 语义 / Toast /
> Confirm 对话框 / `get_items` 拉取 / `items_changed` 100ms 合并全量拉取；**24/24 单测**（含
> P1 修复 2 条回归）+ 工程验收全绿，全 workspace 构建无回归）。真机验收第一轮 5/10 通过，
> 其后 7 轮修复（Toast 渲染裁剪 → 滚动跟随 → 键鼠冲突 → 滚顶回归，见 §4.1–§4.4）已落地
> （13:38 版 `dd-gui.exe`），**已决策按 §4 清单全量复验后关闭**（2026-09-02）。
>

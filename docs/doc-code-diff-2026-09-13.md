...
### P-01 ｜ 边界条件 ｜ 超限消息的处置两侧都未实现 ✅（2026-09-15 已闭环，见 §12）
...
- **文档**：用户承诺红线——**A-33-06 / A-33-08 通过前，不得把「无需 `es.exe`」/「Everything 运行中即可用」写入用户文档**。（**2026-09-19 更新：该红线已由用户决策解除** —— A-33-06 已两分支通过，用户文档 `search.md` v1.3 起改为「Everything 在运行即可用，`es.exe` 为可选回落」；A-33-08 剩余环境以「直连不可用时回落」表述承接。）
...
|---|---|---|---|---|---|
| P-02 | 数据结构 | `protocol.md:142` | `jsonrpc` 缺失或非 `"2.0"` → `-32600` | `process.rs:596-601`：非 `"2.0"` 入 `unmatched` 忽略；缺失致反序列化失败 → `MalformedEnvelope` 冒泡**中止整个 `call`** | 均不回 `-32600` —— ✅ **已解决**（2026-09-15，见 §12） |
| P-03 | 数据结构 | `protocol.md:143,151` | `id` 限非负整数；类型非法 → `-32600` | `messages.rs:43`：`id: Option<u64>`；负/字符串 id 反序列化失败 → `MalformedEnvelope` 冒泡 | 不回 `-32600`；`id=0` 合法、上界 `u64::MAX` —— ✅ **已解决**（2026-09-15，见 §12） |
| P-04 | 边界条件 | `protocol.md:158` | 收到数组批量请求 → 回 `-32600` | host：反序列化失败冒泡（不回）；ext `serve_line`：回 **`-32700`** | 两侧都不回 `-32600` —— ✅ **已解决**（2026-09-15，见 §12；扩展侧误回的 `-32700` 亦已纠正） |
...
| S-11 | ✅ 已消除 | `search-file.md` §4 | 原：标题称「A-33-05…A-33-10 唯一未闭环」而表内仅列 05/06/07/08/10。2026-09-15 改写后标题与表一致（显式列 05/06/07/08/10，09 并入 CI 另注） |
...
| ID | 位置 | 存疑内容 |
|---|---|---|
| P-15 | `protocol.md:146-147` | `result`/`error` 并存时"是否违规"无法判定（代码未强制、未报错） —— ✅ **已解决**（2026-09-15，见 §12：已按 §3.2 判为 Invalid Request 回 `-32600`） |
...
| P-01 | `protocol.md:96` | 超限后「关闭连接」是否真需实现（宿主 `call` 与 `poll` 两条路径都要改） —— ✅ **已解决**（2026-09-15，见 §12） |
...
- `search-file.md` 的依赖清单（`everything-ipc = "=0.1.4"` + `default-features = false`、`fuzzy-matcher`、`chrono`、`windows-sys 0.61`）、`ES_TIMEOUT = 1200 ms`、探活条件 `is_ipc_available() + is_db_loaded()`、~~`f ` 前缀进页~~（**2026-09-19 已移除**，见 §14）与 200 ms 去抖、`resolve_file_url_to_path` 往返均一致；历史归档两份文档的最终处置结论亦与代码现状一致。
...
**未修 / 留人工确认**：~~P-01（是否实现「回错 + 关连接」）~~ ✅ 已于 2026-09-15 由代码侧闭环（见 §12）、~~P-18（方法名常量层属代码重构，已登记 INDEX.md §5 未闭环项）~~ ✅ 已于 2026-09-14 由代码侧闭环（见 §11）、§5 存疑项中的设计决策（M-12/M-13/M-14/M-16/E-06/E-07 等已加现状注记，最终口径待人工定夺）。
...
| 清单条目 | 差异（原文档要求） | 变化 |
|---|---|---|
| **P-02** | §3.2 `jsonrpc` 缺失 / 非 `"2.0"` → `-32600` | ✅ 两侧实现（宿主侧原为静默忽略 / `MalformedEnvelope` 冒泡中止整个 `call`） |
| **P-03** | §3.2 `id` 类型非法 → `-32600` | ✅ 两侧实现（原致反序列化失败并冒泡） |
| **P-14** | §3.2 `params` 非对象、`method` 非字符串 → `-32600` | ✅ 两侧实现（原未校验，数组亦被接受） |
| **P-15** | §3.2 `result` 与 `error` 互斥 → `-32600` | ✅ 两侧实现（原以 `error` 优先、不报错） |
| **P-04** | §3.4 批处理数组 → `-32600` | ✅ 两侧实现（扩展侧原误回 `-32700`） |
| **P-01** | §9.3 `-32600`（超上限支）应为致命 | ✅ 已按致命处置（回错后关连接） |

> 上表 6 条与本清单各表逐行对应：P-01 在 §2 详细条目、P-02/P-03/P-04 在 §3 表、P-14/P-15 在 §4 表、P-01/P-15 在 §5 表，**各处均已标注 ✅**。

**反直觉但必须的实现决策**：`"id": null` **判为合法**（视同「无 id」）。本协议自身的 `-32600`/`-32700` 响应在无法确定 id 时即为此形状（§2.3）；若判为非法，对端收到的是**合法**错误响应却被再次判非法并回错，往返互喷形成自激回路。已在 `dd-protocol::envelope` 与 `dd-ext` 双侧加回归测试锁定。

**协议影响**：无。方法名、字段、错误码取值与章节均未变；`protocol.md` 仅在 §2.3 / §3.2 / §3.4 / §9.3 更新「实现现状」注记（不含语义变更），v1.0 冻结契约不变。

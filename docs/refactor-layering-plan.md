...
> **状态**：已落地 ｜ **版本**：v1.0 ｜ **最后更新**：2026-09-13
...
| 项目 | 状态 | 落点文件 | 备注 |
|---|---|---|---|
| 方案 1（app/ui/platform/text 分层） | ✅ | `crates/dd-gui/src/` | 已实施，三档对比见 §3 |
| 方案 0（保守，仅搬自由函数） | ❌ | — | 未采用 |
| 方案 2（`PaletteCore` 抽离） | ❌ | — | 远期项，未实施（见 §7 N9） |
| `app/`（13 文件）业务编排层 | ✅ | `crates/dd-gui/src/app/` | `mod.rs` 等 13 个模块 |
| `ui/`（11 文件）绘制层 | ✅ | `crates/dd-gui/src/ui/` | 含 `chrome.rs`（原方案列 10 个，补 1 个） |
| `platform.rs` / `text.rs` | ✅ | `crates/dd-gui/src/` | 系统副作用 / 纯函数 |
| `main.rs` 瘦身为入口 | ✅ | `crates/dd-gui/src/main.rs` | 5123 → ~150 行 |
| fmt / clippy / test 三项全绿 | ✅ | — | 221 passed / 0 failed |
| 真机验收 R1–R14 | ⚠️ | — | 编译与单测无法覆盖 UI 行为，待人工过一遍 |

> 本方案核心约束：**纯物理重组、对原有业务逻辑与 UI 界面逻辑零影响**（行为逐字节等价）。
> 本稿为落地后整理：正文已与实现同步，旧方案稿的「源行号」均为拆分前 `main.rs`（5123 行）的历史溯源，当前 `main.rs` 已瘦身为 ~150 行入口，下文涉及旧行号处均已标注为约值。
...
| 3241–3881 | 自由函数：绘制原语等 | 混杂 | 641 |
| 3882–3949 | `impl eframe::App` | 入口 | 68 |
| 3950–4484 | 页脚/键帽/卡片绘制原语 | UI 绘制 | 535 |
| 4485–5123 | `#[cfg(test)] mod tests`（27 个用例） | 测试 | 639 |

### 2.4 混杂的具体证据（拆分前 `impl PaletteApp` 内，约值）
...
| 窗口 | `show` `hide` `dismiss` `hide_keep_state` `poll_hotkey` `poll_tray` `send_center_on_cursor` `handle_focus_loss` | 648 / 671 / 683 / 692 / 698 / 713 / 754,801 / 808 |
...
│   └── chrome.rs           # 无边框窗口 chrome：全窗拖拽 + 8 方向边缘缩放（设计稿 v4.10 D36 / v4.11 修正）
│
├── platform.rs             # ★ 系统副作用：send_center_on_cursor / run_as_admin / reveal_in_folder / setup_cjk_fonts
...
> 更正：目标结构 `ui/` 实际落地 **11 个文件**（原方案稿列 10 个，遗漏 `chrome.rs`）。`chrome.rs` 负责无边框窗口的拖拽与边缘缩放，属绘制层，已随方案 1 落地。
...
| main.rs 剩余 | ~3900 行 | **~150 行** | ~55 行 |
| 业务/UI 边界 | 部分 | **清晰（模块级强制）** | 极清晰（编译期强制） |
| 字段可见性改动 | 无 | **需 35 处 `pub(crate)`** | 需所有权重构 |
| 行为变更风险 | 极低 | **低（可机械验证）** | **高（触碰业务逻辑）** |
| 是否违反"零影响"约束 | 否 | **否** | **是** |
| 建议 | 兜底 | **本次采用** | 列为远期项，见 §7 |

**推荐方案 1 的理由**：它是唯一能在「不触碰任何业务逻辑」前提下达成模块级强制边界的档位。
方案 2 需要重写状态所有权与消息流，必然改动业务逻辑，与本次硬约束冲突。

### 4.2 为什么方案 1 能做到「零行为影响」

`impl PaletteApp { … }` 可以物理拆到 `app/lifecycle.rs`、`ui/panel.rs` 等多个文件里，**只要它们在同一个 crate 内**。Rust 的方法解析在**编译期**完成、不存在重载与动态分发，因此「方法体与调用点一字不改、仅改变它们在磁盘上的位置」在语义上是恒等变换。

> 这是本方案**不把绘制方法改成自由函数**的原因：改成自由函数就要改 60+ 处调用点，那才是真正的风险来源。保持 `impl` 块 = 调用点零改动。

### 4.3 唯一的结构性改动是可见性（`pub(crate)`）

`struct PaletteApp` 的 35 个字段目前是私有的（仅 main.rs 可见）。拆分后 `app/lifecycle.rs`、`ui/panel.rs` 需要读写这些字段 → 字段加 `pub(crate)`。
...
### 4.4 类型定义在 lib，bin 只留 `main()`

`PaletteApp` 移入 `dd_gui::app`（lib crate），`main.rs` 通过 `use dd_gui::app::PaletteApp;` 引用。bin 与 lib 同属一个 package，编译产物与运行时行为一致。

### 4.5 类型系统兜底

任何漏搬、错搬、签名错位都会导致**编译失败**而非运行时偏差；任何逻辑改动都无法被"移动"掩盖——因为方法体是原样搬运，diff 可逐行审计。

---

## 5. 精确切割表（历史溯源，源自我之前 main.rs）

> 以下行号为**拆分前 `main.rs`（5123 行）的约值**，仅作迁移溯源；当前 `main.rs` 已瘦身为 ~150 行入口，符号已分布于 `app/` `ui/` `platform.rs` `text.rs`。搬运单位 = 完整函数/常量/结构体定义块（含其文档注释），**函数体一字不改**。

### 5.1 批次 1：`text.rs`（纯函数，零 egui 依赖）
...
### 5.2 批次 2：`platform.rs`（系统副作用）

| 源行（约） | 符号 | 目标 |
|---|---|---|
| 356 | `setup_cjk_fonts` | `platform.rs` |
| 754 / 801 | `send_center_on_cursor`（双 cfg 成对搬） | `platform.rs` |
...
| 3249 | `ctx_entry_count` | `ui/context_menu.rs`（实际落 `app/ctx_menu.rs`，见 §9.3） |
| 3270 | `context_menu_rows` | `app/ctx_menu.rs` |

### 5.4 批次 4：`app/` 骨架 + 数据结构
...
### 5.5 批次 5：`impl PaletteApp` 业务方法拆分
...
### 5.6 批次 6：`impl PaletteApp` 绘制方法拆分
...
| `refresh_health_drops` `consecutive_crashes` `poll_invoke_on_dead` | 3 | `app/health.rs` / `app/invoke.rs` |
...
> 与 v1 基线（219）相比：本轮 +2。变化来源——新增 `tray.rs`（设计稿 10C）使 dd-gui lib 测试 76→81（+5）；dd-ext `system` bin 测试 8→5（-3）。验收要求仍是 **0 failed**，迁移后总数守恒（221）。
...
| # | 决策点 | 结论 |
|---|---|---|
| Q1 | 拆分力度 | **方案 1**（main.rs → ~150 行），已实施 |
| Q2 | `PaletteApp` 落点 | **`dd_gui::app`（lib，可单测、main.rs 最薄）**，已实施 |
| Q3 | 交付节奏 | **8 批次逐批验证后交付**，已实施 |
| Q4 | 方案 2（远期） | 未实施，列为后续里程碑 |
...
| 项 | 执行前 | 执行后 |
|---|---|---|
| main.rs | 5123 行（混杂业务+绘制） | **~150 行**（仅文件头 + `main()`） |
| dd-gui 模块 | 13 个既有模块 | 13 个既有模块（零改动）+ `app/`(13 文件) + `ui/`(11 文件) + `platform.rs` + `text.rs` + `test_support.rs` |
...
原 27 个 bin（dd-run）测试按 §5 映射表迁入 lib 模块（`test_support.rs` 承载共享夹具 `item_with` / `dying_ext` / `dying_process` / `make_app` / `ctx`，内容逐字未改）：

text.rs 7、ui/icons.rs 3、ui/states.rs 2、app/ctx_menu.rs 6、app/fallback_flow.rs 4、app/health.rs 2、app/invoke.rs 1、app/toast.rs 2（合计 27 ✅）。

迁移后分目标计数：**dd-gui bin 27→0，dd-gui lib 81→108**，其余目标不变，总计 221 守恒。

### 10.5 可见性改动（纯标注，编译器保证语义不变）

- `struct PaletteApp` 35 个字段 → `pub(crate)`（方案 §4.3）；
- 数据结构（AggregatePayload / InvokeOutcome / PageOutcome / FallbackFetchOutcome / ToastState / ConfirmDialog / CtxEntry / CtxMenuState / RefreshState / KeyGroup）字段 → `pub(crate)`（跨模块构造/绘制所需，方案低估了这部分，同性质：纯可见性）；
- 全部 `impl PaletteApp` 固有方法 → `pub(crate)`；bin 需要的入口 → `pub`（`PaletteApp` / `new` / `spawn_aggregation` / `AggregatePayload` / `APP_W` / `APP_H` / `OFFSCREEN_X` / `OFFSCREEN_Y` / `setup_cjk_fonts`）。

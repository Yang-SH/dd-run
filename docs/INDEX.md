...
> **状态**：生效中 ｜ **版本**：v1.30 ｜ **最后更新**：2026-09-24
...
| [`implementation.md`](./implementation.md) | 生效中 | v0.1.6 | 963 | 维护者 | 实施主线：里程碑状态总表、目标与范围、ADR-1~4、验收映射（含 §3.1 测试基线台账）、遗留台账、实施沿革 |
...
| [`security-audit-2026-09-23.md`](./security-audit-2026-09-23.md) | 生效中（v1.7） | 763 | **全量代码安全审计与修复方案 + 修复记录**：79 个源文件（35,975 行）按攻击面分组走查，确认 11 项缺陷（**1 高 / 5 中 / 5 低**）+ **14 项已核验安全项**；**已修复 7 项** —— S-01（命令注入，§3.1.1）、S-02（NDJSON 无界缓冲，§4.1.1）、S-03（`open_url` scheme 白名单，§4.2.1）、S-04（图标读盘+解码双上限，§4.3.1）、**S-05（扩展信任门禁，§4.4.1–§4.4.3）**、S-06（危险命令二次确认，§4.5.1）、S-10（`entry.env` 关键变量保护，§5.1）；**余 4 项低危**：S-07/S-08/S-09/S-11；含威胁模型与信任边界（§2.3）、逐项修复代码与验收判据、取证脚本（§8）、**「批量 spawn 用例失败（`error 231`）」的排查留档（§7.2，已定性为环境瞬时限制并复跑销项）** |
...
| 代码安全审计 11 项发现（**已修复 7 项** ✅ / 余 **4 项低危**） | [`security-audit-2026-09-23.md`](./security-audit-2026-09-23.md) §1、§7 | 🟨 **大部分销项**：**已修复** S-01（高，命令注入）、S-02（中，NDJSON 无界缓冲 —— PoC 已反转为 bounded）、S-03（中，`open_url` 三 scheme 白名单）、S-04（中，图标读盘/解码双上限）、**S-05（中，扩展信任门禁 —— 2026-09-24 实施，见该文档 §4.4.3：`dd-host/src/trust.rs` + spawn 唯一入口门禁 + 设置页行内审批；A11 哈希开销实测 0.874 ms @836,608 B）**、S-06（中，危险命令二次确认）、S-10（低，`entry.env` 关键变量保护）= **7 项 / 共 +35 单测**（S-03 含实施后自查补的字节切片 panic 护栏；S-05 新增 19 条）。**余 4 项低危**：S-07（剪贴板静默写）/ S-08（`explorer` raw_arg）/ S-09（缓存键碰撞）/ S-11（`serve_line` 的 `.expect` 面）。⏳ 全仓 `cargo test --workspace` 本轮 = **487 passed / 21 failed**，其中 **20 条为已知 `Os error 231`（`ERROR_PIPE_BUSY`）环境批**（失败名单无 S-05 新测试）+ 1 条既有**机器绑定**用例，环境自愈后预期 **507 passed / 1 failed**；进度同步登记于 [`implementation.md`](./implementation.md) §6.1 台账 L11 |

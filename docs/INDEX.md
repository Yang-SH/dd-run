...
> **状态**：生效中 ｜ **版本**：v1.11 ｜ **最后更新**：2026-09-17
...
| [`implementation.md`](./implementation.md) | 生效中 | v0.1.1 | 712 | 维护者 | 实施主线：里程碑状态总表、目标与范围、ADR-1~4、验收映射（含 §3.1 测试基线台账）、遗留台账、实施沿革 |
...
| 文档 | 状态 | 行数 | 职责 |
|---|---|---|---|
| [`search-file.md`](./search-file.md) | 生效中（v3.5） | 930 | **文件搜索的权威技术口径**：双通道传输层、依赖、落地范围、两轮真机验收结论、A-33-05 阈值修订（v3.4）、回落失败不静默（v3.5）、版本演进 |
...
| [`doc-code-diff-2026-09-13.md`](./doc-code-diff-2026-09-13.md) | 生效中 | 339 | **文档 ↔ 代码差异清单与修复记录**（71 条差异，三路独立复核验真后按「文档对齐代码」逐条落实修复，含每条修复落点）；§10–§13 记代码侧闭环 **9 项**（P-01/P-02/P-03/P-04/P-06/P-13/P-14/P-15/P-18）与两项协议口径定稿，遗留待人工确认项见其 §5 |
...
| [`../CHANGELOG.md`](../CHANGELOG.md) | 生效中 | 239 | 版本变更日志（`[0.1.0]`/`[0.1.1]` 条目均带日期；`[Unreleased]` 含 2026-09-17 四项修复/变更） |
...
| 文件搜索 P2 真机验收 A-33-05…A-33-10 | [`search-file-p2-acceptance-2026-09-15.md`](./search-file-p2-acceptance-2026-09-15.md) | ✅ 两轮完成（09-15 首轮 / **09-17 补 `es.exe`**，报告 v1.3）：**A-33-05 / A-33-06 / A-33-07 / A-33-10 通过**（基线 p95 324.21ms → 降幅 91.27%；回落分支 3/3 返回真实结果；恢复判据改为通道级）；**A-33-08 ⚠️ 部分**（跨环境矩阵）。**3 项未闭环**见报告 §5（A-33-08 矩阵、GUI 端到端 ≤200ms、A-33-03 的 `%`/`#` 打开）。**红线收窄为 A-33-08**：用户文档是否解除 `es.exe` 前置表述属**待决策项** |

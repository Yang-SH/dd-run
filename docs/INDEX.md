...
> **状态**：生效中 ｜ **版本**：v1.1 ｜ **最后更新**：2026-09-14
...
flowchart TD
    ...
    PLAN["专题方案<br/>search-file / refactor / apps / memory / icons"] --> IMPL
...
| 文档 | 状态 | 版本 | 行数 | 读者 | 职责 |
|---|---|---|---|---|---|
| [`protocol.md`](./protocol.md) | 已冻结 | v1.0 | 779 | 宿主与扩展作者 | 进程间通信：JSON-RPC over NDJSON、生命周期、8 种执行结果、错误码、超时 |
...
| [`implementation.md`](./implementation.md) | 生效中 | v0.1.1 | 438 | 维护者 | 实施主线：里程碑状态总表、目标与范围、ADR-1~4、验收映射、遗留台账、实施沿革 |
...
| [`optimization-plan.md`](./optimization-plan.md) | 规划中 | 137 | **项目可优化方案总览（增量）**：体积/协议健壮性/方法名常量/可观测性/依赖治理/跨平台，含已证伪项 |
...
| [`doc-code-diff-2026-09-13.md`](./doc-code-diff-2026-09-13.md) | 生效中 | 273 | **文档 ↔ 代码差异清单与修复记录**（71 条差异，三路独立复核验真后按「文档对齐代码」逐条落实修复，含每条修复落点）；遗留待人工确认项见其 §5 |
...
| §1.3 方法名无常量层（字符串字面量分发） | `dd-ext`/`dd-host`/`dd-gui` 分发点 | ⚠️ 工程改进建议，未修（差异清单 P-18） |

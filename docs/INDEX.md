...
> **状态**：生效中 ｜ **版本**：v1.10 ｜ **最后更新**：2026-09-16
...
| 文档 | 状态 | 版本 | 行数 | 读者 | 职责 |
|---|---|---|---|---|---|
| [`protocol.md`](./protocol.md) | 已冻结 | v1.0 | 784 | 宿主与扩展作者 | 进程间通信：JSON-RPC over NDJSON、生命周期、8 种执行结果、错误码、超时 |
...
| [`implementation.md`](./implementation.md) | 生效中 | v0.1.1 | 656 | 维护者 | 实施主线：里程碑状态总表、目标与范围、ADR-1~4、验收映射、遗留台账、实施沿革 |
...
| 文档 | 状态 | 行数 | 职责 |
|---|---|---|---|
| [`search-file.md`](./search-file.md) | 生效中（v3.4） | 926 | **文件搜索的权威技术口径**：双通道传输层、依赖、落地范围、首轮真机验收结论、A-33-05 阈值修订（v3.4）、版本演进 |
...
| [`doc-code-diff-2026-09-13.md`](./doc-code-diff-2026-09-13.md) | 生效中 | 327 | **文档 ↔ 代码差异清单与修复记录**（71 条差异，三路独立复核验真后按「文档对齐代码」逐条落实修复，含每条修复落点）；§10–§12 记代码侧闭环 **8 项**（P-01/P-02/P-03/P-04/P-06/P-14/P-15/P-18），遗留待人工确认项见其 §5 |
...
| [`search-file-p2-acceptance-2026-09-15.md`](./search-file-p2-acceptance-2026-09-15.md) | 生效中 | 202 | **文件搜索 P2 真机验收报告**：A-33-05/06/07/08/10 逐项实测（环境矩阵、耗时分解、1000 次基准、故障注入与恢复链路），含未闭环项与复现命令；v1.1 依扩展内**分阶段计时日志**更正耗时分解，v1.2 记 **A-33-05 阈值修订决策**（§4.1.1） |
...
| [`../CHANGELOG.md`](../CHANGELOG.md) | 生效中 | 206 | 版本变更日志（`[0.1.0]`/`[0.1.1]` 条目均带日期） |
...
| 文件搜索 P2 真机验收 A-33-05…A-33-10 | [`search-file-p2-acceptance-2026-09-15.md`](./search-file-p2-acceptance-2026-09-15.md) | 🟨 首轮完成（2026-09-15；2026-09-16 定因插桩 + **阈值修订**）：A-33-07/10 ✅、A-33-06/08 ⚠️ 部分、**A-33-05 修订后达标**（原阈值低于 Everything 引擎地板 11–25ms，扩展自身 ≲1ms）。**4 项未闭环**见报告 §5（`es.exe` 前置、GUI 端到端、`%`/`#` 打开、A-33-08 矩阵）。**A-33-06/08 未全通过前，用户文档仍不得承诺「无需 es.exe」** |

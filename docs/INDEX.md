...
> **状态**：生效中 ｜ **版本**：v1.15 ｜ **最后更新**：2026-09-19
...
| [`implementation.md`](./implementation.md) | 生效中 | v0.1.1 | 774 | 维护者 | 实施主线：里程碑状态总表、目标与范围、ADR-1~4、验收映射（含 §3.1 测试基线台账）、遗留台账、实施沿革 |
...
| 文档 | 状态 | 行数 | 职责 |
|---|---|---|---|
| [`search-file.md`](./search-file.md) | 生效中（v3.8） | 1004 | **文件搜索的权威技术口径**：双通道传输层、依赖、落地范围、两轮真机验收结论、A-33-05 阈值修订（v3.4）、回落失败不静默（v3.5）、`Ctrl+F` 直达与 Shell 真实图标（v3.6，含两处未达标处置选项 §10.4）、**移除 `f ` 前缀直达（v3.7）**、**解除 `es.exe` 前置表述（v3.8）**、版本演进 |
...
| [`search-file-ctrl-f-icons-plan.md`](./search-file-ctrl-f-icons-plan.md) | 已落地（两处 ⚠️ 待决策；v1.2 修订：`f ` 前缀直达已移除） | 283 | **文件搜索开启方式与图标方案**：`Ctrl+F` 直达（决策 D1-x）与 Shell 真实文件图标（决策 D2-x）的决策留痕；§11 = 落地记录与 A-CF/A-IC 实测（**按真实路径档首抽 703.8 ms、sidecar +104.5 KB 未达标**） |
...
| [`../CHANGELOG.md`](../CHANGELOG.md) | 生效中 | 276 | 版本变更日志（`[0.1.0]`/`[0.1.1]` 条目均带日期；`[Unreleased]` 含 2026-09-19 七批变更——**移除 `f ` 前缀直达** / `Ctrl+F` 直达 + Shell 真实图标 / 模糊排序字段分层 / 应用列表调试工具过滤 / **解除 `es.exe` 前置表述**等） |
...
| 文件搜索 P2 真机验收 A-33-05…A-33-10 | [`search-file-p2-acceptance-2026-09-15.md`](./search-file-p2-acceptance-2026-09-15.md) | ✅ 两轮完成（09-15 首轮 / **09-17 补 `es.exe`**，报告 v1.3）：**A-33-05 / A-33-06 / A-33-07 / A-33-10 通过**（基线 p95 324.21ms → 降幅 91.27%；回落分支 3/3 返回真实结果；恢复判据改为通道级）；**A-33-08 ⚠️ 部分**（跨环境矩阵）。**3 项未闭环**见报告 §5（A-33-08 矩阵、GUI 端到端 ≤200ms、A-33-03 的 `%`/`#` 打开）。**红线收窄为 A-33-08**；**`es.exe` 前置表述已于 2026-09-19 解除**（`search.md` v1.3 起改为「Everything 在运行即可用，`es.exe` 为可选回落」） |

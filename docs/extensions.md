...
### 6.1 宿主的信任门禁：为什么我的扩展没被加载

**清单合法 ≠ 一定被加载**（S-05，2026-09-24 起）。宿主在扫描之后会再做一次**信任判定**，
只有通过的扩展才会被拉起；这与你写的代码无关，但决定了你的调试体验：

| 情形 | 结果 |
|---|---|
| 扩展**随包分发**（清单位于 `dd-run.exe` 同目录的 `extensions.d/`）**且** id 在宿主首方白名单内 | 自动信任（当前白名单仅 `com.ddrun.filesearch`） |
| 扩展放在**用户数据目录**（`%APPDATA%\dd-run\extensions.d\`） | **首次需用户在「设置 › 扩展」里点「允许」**；批准前不会 spawn |
| 你改动了已批准扩展的**清单或 exe 内容** | 回到「待批准」（批准绑定到内容哈希，改动即失效） |
| 用户点了「阻止」 | 持久阻止，需再次「允许」才恢复 |

排查看什么：**设置页「扩展」栏**会列出每个扩展的**来源**（内置 / 随包 / 用户安装）与
**信任状态**（待批准 / 已阻止），并提供「允许 / 阻止」按钮；鼠标悬停来源标签可看到清单与
exe 的完整路径。宿主日志（stderr，见 §8）会打印 `扩展 <id> 未获信任（trust=…, origin=…）`。

两条给作者的建议：① **本地开发**就把扩展放进用户数据目录并点一次「允许」即可，之后只要不改
内容就不会再打扰；② **别指望靠 id 前缀冒充官方的**——判据是「来源 + 白名单」双条件，
`com.ddrun.evil` 这类名字不会被自动信任（这正是 S-05 选型时修掉的判据漏洞）。

> 台账文件 `%APPDATA%\dd-run\trust.json` 是**宿主私有**的，扩展不要读写它；它损坏或被删除时
> 宿主的处理是"全部回到待批准"（fail-closed），而不是放行。

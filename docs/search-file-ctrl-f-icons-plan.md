...
> **状态**：已落地（2026-09-19；**E1 / E2 均已处置达标**）｜ **版本**：v1.4 ｜ **最后更新**：2026-09-19
...
| v1.2 / v1.3 | 2026-09-19 | E1 落地随附（§6.2 新增 A-IC-08/09/10 + 口径变更注；**版本行当时漏登，此处补记**） |
| v1.4 | 2026-09-19 | **E2 落地**：A-IC-06 由 ❌ 转 ✅（零依赖 PNG 编码器，831,488 B / 增量 61,952 B ≤ 64 KB）；A-IC-02 行补 E1 后口径与 02b 压线注记 |
...
| A-IC-02（`icon_ms` 预算） | ✅（02c 同步口径，E1）／⚠️ 02b 压线 | E1 后：缓存命中 0.10 ms ✅、按真实路径档**同步** 0.32 ms ✅；**扩展名档首抽（02b）负载敏感**：E1 批次 40.29 ms（压线）→ E2 后两轮 50.29 / 66.84 ms（判据 ≤40、观察线 60；判因为机器负载、编码单张实测 0.62 ms，见 `search-file.md` §10.6） |
...
| A-IC-06（体积 ≤ 64 KB） | ✅ 已达标（E2，2026-09-19） | 零依赖 PNG 编码器后 **831,488 B**，相对基线 769,536 B 增量 **61,952 B ≤ 65,536 B**（余量 3,584 B 压线）；详见 `search-file.md` §10.6 |
| A-IC-07（非 Windows 编译） | ✅（静态门控审查） | `extract_png` 非 Windows 桩 + `cfg` 门控；未在非 Windows 目标实跑 |

**两处未达标项均已处置达标（2026-09-19）**：E1 = 分层异步 + 自动刷新（[`search-file.md`](./search-file.md) §10.5）、E2 = 零依赖 PNG 编码器（§10.6）；处置决策记录见两节。

...
> **状态**：生效中 ｜ **版本**：v3.3 ｜ **最后更新**：2026-09-13
...
- **唯一未闭环项**：P2 真机验收 **A-33-05…A-33-10** 仍未做（见 §4），其余 A-33-01…A-33-04、A-33-09 已通过或代码已具备。
...
- **P2（✅ 代码已实施）**：`everything-ipc` 主通道 + `es.exe` 回落；L2 冒烟已验证 IPC 主通道真机返回真实结果。**真机验收 A-33-05…A-33-10 待做（§4）**。
...
## 4. 真机验收待办：A-33-05/06/07/08/10（唯一未闭环项）

> ⚠️ **状态：未完成（唯一阻塞项）**。以下为 §7.4 验收标准 A-33-05/06/07/08/10 的待办汇总；A-33-01…A-33-04、A-33-09 已通过或代码已具备。在 A-33-06 / A-33-08 通过前，不得把「无需 es.exe」写入用户文档。

表：待办真机验收项。

| 编号 | 待验收标准 | 通过证据 |
| --- | --- | --- |
| A-33-05 | 扩展进程内 IPC 查询 p50 <10ms、p95 <30ms；相对 `run_es` 基线 p95 降 ≥50%（端到端另设「输入到首屏 ≤200ms」感知指标，分别测量） | 1000 次同机基准 + 端到端计时日志 ⚠️ **待做** |
| A-33-06 | IPC 不可用时 100% 回落 `es.exe`；两者均不可用 100% 返回引导项；单请求总耗时 ≤2000ms | 故障注入报告（含耗时分解）⚠️ **待做** |
| A-33-07 | 连续 1000 次查询无崩溃、线程/句柄持续增长或 PATH_INDEX 超限；RSS 增长 <10% | 进程/线程/句柄/RSS 时间序列 ⚠️ **待做** |
| A-33-08 | Win10/11 × Everything 1.4/1.5 × 完整性级别（普通/提升/服务实例）× 1.5 实例名 支持矩阵全有结果或明确「不支持」；UIPI 受限场景可观测回落 | 真机矩阵报告 + 回落日志 ⚠️ **待做** |
| A-33-10 | IPC 恢复：Everything 退出→重启后 N 次查询内自动回 IPC；长驻进程不得永久停留 es.exe 回落态 | 重启注入 + 通道切换日志时间戳 ⚠️ **待做** |

⚠️ *A-33-03 的 `%` / `#` 实际打开验证随上述一并执行。A-33-09 已并入 CI 门禁；A-33-01…A-33-04 见 §7.4 表。*
...
> **实施状态（2026-09-09/10 更新）**：**P0 ✅**、**P1 ✅**、**P2 ✅ 代码已实施**（everything-ipc 主通道 + es.exe 回落；L2 冒烟验证 IPC 主通道真机生效）；**P2 真机验收 A-33-05…A-33-10 待做（§4）**。`Sender::ContextMenu` / `InvokeContext.selected_item_id` 为协议 v1.0 既有定义，零协议改动兑现。
...
| A-33-05 | IPC 查询 p50 <10ms、p95 <30ms；相对 `run_es` 基线 p95 降 ≥50%（端到端另设「输入到首屏 ≤200ms」） | 1000 次基准 + 计时日志 ⚠️ **待做（§4）** |
| A-33-06 | IPC 不可用 100% 回落 es.exe；均不可用 100% 返回引导项；单请求 ≤2000ms | 故障注入 ⚠️ **待做（§4）** |
| A-33-07 | 连续 1000 次无崩溃/线程句柄增长/PATH_INDEX 超限；RSS 增长 <10% | 时间序列 ⚠️ **待做（§4）** |
| A-33-08 | Win10/11 × 1.4/1.5 × 完整性级别 × 1.5 实例名 矩阵全有结果或明确不支持；UIPI 受限可观测回落 | 真机矩阵 ⚠️ **待做（§4）** |
| A-33-09 | fmt/clippy/workspace 测试/release 打包均 exit 0；协议测试无新增字段 | CI 日志 |
| A-33-10 | IPC 恢复：Everything 退出→重启后 N 次查询内回 IPC；长驻不永久停留 es.exe 回落态 | 重启注入 + 日志 ⚠️ **待做（§4）** |
...
| v3.3 | 2026-09-08~10 | 传输层切 `everything-ipc` 主通道 + `es.exe` 回落（双通道均落地）；P0/P1/P2 代码实施；`path_to_file_url` 取代 `pct_encode`；超时统一 1200ms | ❌ 当前生效（仅 A-33-05…A-33-10 真机验收待做） |

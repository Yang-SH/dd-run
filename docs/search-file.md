...
| 编号 | 结论 | 关键实测 |
| --- | --- | --- |
| A-33-05 | ✅ **通过**（09-17 两半均达标） | IPC 1000 次 p50 **24.156 ms**／p95 **28.300 ms**（对 < 30／< 50）；**`run_es` 基线 p95 324.213 ms → 降幅 91.27 %**（≥ 50 %）。**扩展自身成本 ≲ 1 ms**，耗时几乎全在 IPC 查询往返（＝ Everything 引擎地板）。首轮数据（p50 26.87／p95 31.49）留报告 §4.1 |
| A-33-06 | ✅ **通过**（09-17 两分支实测） | ① 两者均不可用 → **20/20 引导项**（p50 0.143 ms、max 760 ms ≤ 2000 ms）；② IPC 不可用但 es 可用 → **3/3 回落 `es.exe` 且返回 30 条真实结果**（通道 `ipc->es`，total 225–282 ms） |
| A-33-07 | ✅ **通过** | 1000 次查询无崩溃/无异常；RSS 稳态增长 **+5.86 %**（< 10 %）且序列震荡非单调；线程恒 5、句柄恒 159 |
| A-33-08 | ⚠️ **部分覆盖** | 本机单元（Win11 × 1.4 × 非提升 × 默认实例）**有结果**；Win10 / Everything 1.5 / 提升(UIPI) / 1.5 命名实例 **未覆盖** |
| A-33-10 | ✅ **通过** | 三轮注入均恢复，**09-17 起恢复判据为「通道回到 `ipc`」**：轮次 3 第 **8** 次查询／**15.16 s**（期间 3 次 `ipc->es` 回落 → 释放 client → 重建）；长驻进程未停留回落态 |
...
⚙️ **2026-09-17 第二轮（补 `es.exe`）**：装 ES 1.1.0.37 后补跑 `bench` / `fault` —— A-33-05 基线半（降幅 91.27 %）、A-33-06 回落半（3/3 返回真实结果）均达标，**A-33-05 / A-33-06 由「部分」改判「通过」**；同时修掉一个**产品缺陷**：`es.exe` 失败（`rc≠0`，错误仅在 stderr）此前被静默当成「无结果」，现转为可读 `files.error` 项（报告 §4.2.1）。**A-33-10 的恢复判据改为通道级**（旧「有响应」判据会把 es 回落误判为恢复）。
...
| A-33-10 | IPC 恢复：Everything 退出→重启后 N 次查询内回 IPC；长驻不永久停留 es.exe 回落态 | ✅ **通过**：三轮注入均恢复（18.06s／10.55s／**15.16s**），09-17 起判据为**通道回到 `ipc`**——[报告](./search-file-p2-acceptance-2026-09-15.md) §4.5 |
...
- P2 任一故障：关闭 IPC 优先，用 `es.exe` 回落；不得把「Everything 运行中即可用」写入用户文档，直到 **A-33-08**（环境矩阵）通过 —— A-33-06 已于 2026-09-17 两分支实测通过。
...
**决策**：采纳「P1 分层异步 + C1 自动刷新」（用户选定）——**不动** `A-IC-02` 的 40 ms 预算，也不改「每程序自身图标」的键分级决策（Q4-A）。

**做法**（`dd-ext/src/bin/search.rs` + `dd-ext/src/lib.rs`）：

1. **按键分层**：`path:<真实路径>`（`.exe`/`.lnk`/`.msi`/`.url`）**一律不进同步路径** → 立即返回类别 glyph 并把 `(键, 取样路径)` 投递后台；`ext:` / `dir` / `noext` 键同键复用（每查询仅 1–3 个新键）留在同步路径，并受 `ICON_SYNC_BUDGET = 15 ms` 时间预算兜底（余量耗尽同样改投后台）。
2. **后台 worker**：4 线程（首次入队时惰性启动），**取任务时才持队列锁** → 抽图 → 落盘缓存 + 进程内缓存；按键去重（`ICON_INFLIGHT`）。
3. **自动刷新（C1）**：worker 补齐后按 `ICON_NOTIFY_THROTTLE = 200 ms` 节流发 §7.1 `items_changed(files.results)` —— **协议零改动、宿主零改动**（`refresh.rs` 已有「命中当前页 → 100 ms 合并窗口 → 重拉」链路）。
4. **框架支撑**：`dd-ext/src/lib.rs` 新增跨线程通知器（`install_notifier` / `notify_items_changed`）；`run()` 把共享 stdout 以 `Mutex` 包裹注入，`write_all + flush` 在同一临界区完成 → 后台通知与请求响应**不交错**（§2.2）。未安装时为 **no-op**，不影响 5 个内置扩展。

**实测（`tools/icon_acceptance.py --cold`，release sidecar 直驱）**：

| 项 | 修复前 | 修复后 |
| --- | --- | --- |
| A-IC-02c 按真实路径档**同步** `icon_ms` | **191–704 ms** ❌ | **0.32 ms** ✅（本档 `path=0 / glyph=30` = 全部下沉后台，符合设计） |
| A-IC-08 后台补齐后重查（**新增判据**） | — | **`path=30 / glyph=0`，`icon_ms` 0.09 ms** ✅ |
| A-IC-04 `.lnk` 批 | 首查即 `path=17 / glyph=13` | 首查 `glyph=30`（预期）→ 补齐后 **`path=17 / glyph=13`** ✅（不可访问路径回落仍生效） |
| A-IC-02b 按扩展名首抽 | 12.4–49.1 ms | **32.23 ms**（各档 32.23 / 8.33 / 11.11 / 11.00）✅ |
| A-IC-02a 缓存命中 | 0.13–0.44 ms | **0.10 ms** ✅ |
| A-IC-10 稳定性（**新增判据**） | — | 10 次查询 `kind` 全为 `results` ✅ |

**行为变更（须知悉）**：首次查询中 `.exe`/`.lnk`/`.msi`/`.url` 行**先显示类别图标**，约 0.2–0.8 s 后由宿主自动重拉替换为真实图标（用户无需再输入或翻页）。

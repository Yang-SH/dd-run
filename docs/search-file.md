...
| 编号 | 结论 | 关键实测 |
| --- | --- | --- |
| A-33-05 | ❌ **未达标**／🟨 基线无法验证 | IPC 往返 p50 **12.53 ms**（目标 <10）；1000 次查询 p50 **26.87 ms**／p95 **31.49 ms**（目标 <30，实际 10.0% 超 30 ms）；`run_es` 基线需 `es.exe`，本机未安装 |
...
| A-33-05 | IPC 查询 p50 <10ms、p95 <30ms；相对 `run_es` 基线 p95 降 ≥50%（端到端另设「输入到首屏 ≤200ms」） | ❌ **未达标**（IPC 往返 p50 12.53ms；1000 次 p50 26.87／p95 31.49ms）；基线 🟨 阻塞（缺 `es.exe`）——[报告](./search-file-p2-acceptance-2026-09-15.md) §4.1 |
...
| v3.3 | 2026-09-08~10 | 传输层切 `everything-ipc` 主通道 + `es.exe` 回落（双通道均落地）；P0/P1/P2 代码实施；`path_to_file_url` 取代 `pct_encode`；超时统一 1200ms | ❌ 当前生效（A-33-05…A-33-10 首轮真机验收已完成，见 §4） |

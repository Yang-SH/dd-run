...
- ✅ **实现现状（2026-09-15）**：两侧均已按本节实现——收到超限帧即回 `-32600`（`id: null`，`data.reason = message_too_large`）并**关闭连接**：宿主由 `process.rs::abort_oversized` 终止子进程，扩展由 `dd-ext::run` 退出。帧层判定出自 `dd-protocol::framing` 的 `Frame::TooLarge`，错误响应形状收口于 `dd-protocol::envelope`。详见 [optimization-plan](./optimization-plan.md) §2.2.1。
...
- ✅ **实现现状（2026-09-15）**：两侧日志统一经 `log` facade + `dd-protocol::logging` 的 stderr 后端输出（**后端恒写 stderr**，从机制上排除污染 stdout 协议通道的可能）；级别与 target 过滤用环境变量 `DDRUN_LOG`（默认 `debug`，与接入前逐行等价）。宿主对扩展 stderr 的捕获与崩溃诊断见 `ExtensionProcess::failure_detail`。详见 [optimization-plan](./optimization-plan.md) §2.4.1。
...
> ✅ **实现现状（2026-09-15）**：上表各条均已在两侧接线（共享 `dd-protocol::envelope`）——`jsonrpc` 缺失 / 非 `"2.0"`、`id` 类型非法、`params` 非对象、`result`+`error` 并存、`method` 非字符串一律回 `-32600`，并在 `data.reason` 标明具体 kind；`id` 能解析时回带上，否则为 `null`（§2.3）。
>
> ⚠️ 其中 **`"id": null` 判为合法**（视同「无 id」）：本协议自身的 `-32600`/`-32700` 响应在无法确定 id 时即为此形状，若判为非法，对端收到**合法**错误响应会被再次判非法并回错，往返互喷形成自激回路。已在 `envelope::tests` 与 `dd-ext` 双侧锁定。详见 [optimization-plan](./optimization-plan.md) §2.2.1。
...
> ✅ **实现现状（2026-09-15）**：两侧均已回 `-32600`（`data.reason = batch_array`）——扩展侧此前误回的 `-32700` 已纠正。判定收口于 `dd-protocol::envelope`，与宿主侧同一份规则。详见 [optimization-plan](./optimization-plan.md) §2.2.1。
...
> ⚠️ **实现现状（核对至 2026-09-14）**：宿主**无脏标记机制**——不可见页的通知被直接丢弃（`refresh.rs:43`）；命中当前页的通知也进入 100 ms 合并窗口，并非「立即」拉取。（2026-09-14 更正：in-process 路径的通知队列此前只增不清、同一 `page_id` 会被反复上报；现 `poll_notifications` 已改为**消费式取走**（`std::mem::take`），与子进程路径的一次性消费一致。）
...
- 错误**不是**致命的：收到错误响应后连接应继续保持（除 `-32600` 消息超上限与 `-32004` 版本不兼容外）。✅ **实现现状（2026-09-15）**：**「消息超上限」这一支已按致命处置**（回 `-32600` 后关闭连接，见 §2.3 现状）；其余 `-32600` 触发（批处理 / 非法 `jsonrpc` / `id` 非法 / `params` 非对象 / `result`+`error` 并存）**非致命**，回错后连接保持（见 §3.2、§3.4 现状）；`-32004` 在扩展侧仍无产出点（见 §5.3 现状）。
- 对端**不得**因为一次错误就退出进程。

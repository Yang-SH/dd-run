...
**当前状态（2026-09-08，M7 批次 7.5 便携 sidecar 扫描，未 commit）**：**「解压即用」关键缺口修复 + zip 布局本机演练**。① **缺口暴露**：7.5 演练按定案布局打 zip（`dd-run-<版本>.exe` + `extensions.d/`），但宿主 `load_extension_sources` 原只扫描用户数据目录 `%APPDATA%\dd-run\extensions.d`（manifest-schema §2 仅一表），exe 旁 sidecar **不被扫描**——「解压即用」不成立（清单在包里，宿主在别处找）。② **修复（`aggregator.rs`）**：`load_extension_sources` 新增**宿主 exe 同目录 `extensions.d/` 便携扫描**——`sidecar_extensions_dir()`（`current_exe.parent()`/`extensions.d`）+ `merge_sidecar_scan()`/`merge_scanned_dirs()`（两处按 id 去重：**用户目录优先**覆盖分发版，sidecar 独有追加；内置仍最优先）；`push_note` 复用于 note 拼接，目录缺失经 `scan_dir` 视作空（非错误），开发期/内嵌物化目录无 sidecar 时零影响。③ **规范同步**：`manifest-schema.md` §2 增「便携 sidecar（v1.0 补充，M7 批次 7.5）」——位置（exe 同目录）、优先级、`${EXT_DIR}` 语义。④ **测试**：aggregator 单测 +2——`merge_scanned_dirs_user_dir_wins_and_sidecar_appends`（同 id 去重 + sidecar 追加顺序 + 用户版保留）/ `merge_scanned_dirs_empty_sidecar_keeps_user_order`；`LoadedExtension` 可构造（manifest.rs 全字段公开）。**验证**：fmt 干净、clippy `-D warnings` 0、dd-gui lib **143 通过 / 0 失败**、全 workspace（CI 同口径 skip steam）**全绿 / 0 失败**。**release.yml 已备**（上轮免安装定案时写入）：tag → `package.sh` → `Compress-Archive` 打 `dist/dd-run-<版本>-portable.zip`（exe + extensions.d）→ `action-gh-release` 附着。**剩余（真机）**：解压 zip 到无扩展环境 → 直接运行确认 file-search 出现（页脚无「扩展目录不可读」、`f ` 前缀可进）、升级覆盖；tag 演练发版一次关账 7.5；M6 集中回归清单照旧。**未 commit**。

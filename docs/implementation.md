...
| M7 发布工程（2026-09-08 立项） | 🚧 7.1 CI ✅（commit `a789906`，首跑绿灯 4m23s）/ 7.2 256px 图标 ✅ / 7.3 ⛔ 取消（仅免安装分发）/ 7.4 ✅ sidecar 定案 / 7.5 🟨 代码完成（commit `818d1db`），真机解压走查 + tag 演练待做 | 见下方 M7 章节 |
...
| 7.5 发布流程 | tag → `package.sh` 产物 + GitHub Release 附着（dist 单文件 + sidecar 目录 zip）；**便携 sidecar 扫描落地**（宿主扫描 exe 同目录 `extensions.d/`，绿色包解压即用） | code：fmt/clippy/全 workspace 测试绿（+2 sidecar 合并单测）+ zip 布局本机验证；**真机解压即用走查 + tag 演练发版待做** | 🟨 代码完成（2026-09-08），真机/tag 演练待做 |

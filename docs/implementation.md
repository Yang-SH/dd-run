...
> **单文件分发（2026-09-04 实施）**：`tools/package.sh` 产出单文件 `dist/dd-run-0.1.1.exe` —— 5 个内置扩展 exe 经 `dd-gui` build.rs + `assets/embed/` 内嵌进宿主，运行时由 `dd-gui::embedded` 物化到 `%APPDATA%/dd-run/cache/embedded/` 再 spawn（进程隔离 ADR-1 不变）。对外入口产物由 `dd-gui` 更名 `dd-run`；M0 CLI 更名 `dd-run-cli`。
...
| M6 日常打磨（2026-09-05 立项，方向经用户确认） | ✅ 已关闭（2026-09-08，M6 集中回归清单 A–F 六组真机验收全部通过，见 §7 最新条目） | 拼音搜索（L4）、冷启动字体后台加载（L10）、设置三子页功能化（热键重绑/开机自启/扩展管理）、L2/L3/L7/L9 销项、三子页 UI 优化（v4.8/v4.9） |
| M7 发布工程（2026-09-08 立项） | 🚧 7.1 CI ✅（commit `a789906`，首跑绿灯 4m23s）/ 7.2 256px 图标 ✅ / 7.3 ⛔ 取消（仅免安装分发）/ 7.4 ✅ sidecar 定案 / 7.5 🟨 tag 演练 ✅（v0.1.1 已发布 2026-09-10），仅剩真机解压走查 | 见下方 M7 章节 |
| M8 扩展生态验证（2026-09-10 立项） | ✅ 8.1 开发指南 ✅ / 8.2 非 Rust 全表面示例 ✅ / 8.3 `dd-run-cli --conformance` ✅ / 8.4 端到端实证 ✅（**10 项全绿**）/ **真机验收 ✅（2026-09-10 用户确认：GUI 内示例命令全部正常执行，含此前报 `missing field kind` 的 invoke 类命令）** | 见下方 M8 章节 |
...
按 2026-09-05 既定路线（日常好用 → 发布工程 → 扩展生态验证）进入 M7：把项目从「能自用」推向「能分发」。M6 剩余真机验收项不阻塞本里程碑开发，以「M6 集中回归清单」（见 §7 当前状态 2026-09-08 条目）集中关账——**A–F 六组已于 2026-09-08 全部真机验收通过，M6 正式关账**（见 §7 最新条目）。

| 批次 | 内容 | 验收标准 | 状态 |
|---|---|---|---|
| 7.1 CI | GitHub Actions（`.github/workflows/ci.yml`，windows-latest + windows-gnu 工具链）：`cargo build --workspace --all-targets`、`cargo test --workspace`（显式跳过机器相关的 apps steam 测试）、`cargo clippy --workspace --all-targets -- -D warnings`、`cargo fmt --all --check`；`CARGO_INCREMENTAL=0`（项目惯例） | push/PR 触发，全 job 绿（首次云端运行验证后关闭） | ✅ 2026-09-08 关账（首跑 run #1，4m23s，全绿） |
| 7.2 256px 图标 | `tools/gen_icon.py` SIZES 增 256 档（LANCZOS、BMP 条目，与既有档同格式 `LoadImageW` 全版本兼容），重新生成 `assets/app.ico`；托盘 ICO 容器守卫测试 5→6 档 | ICO 含 16/20/24/32/48/256 六档；`cargo test -p dd-gui` 全绿 | ✅ 2026-09-08 |
| 7.3 安装器 | ~~Inno Setup 脚本（单文件 + `extensions.d/` sidecar → 安装目录、开始菜单快捷方式、可选开机自启、卸载清理）~~ **按用户决策取消（2026-09-08）：仅提供免安装单文件绿色版**——脚本（`tools/installer.iss` + `tools/build_installer.cmd`）曾完整落地并通过静默安装/卸载冒烟，随决策整体移除，不留安装器代码 | ——（决策记档于 M7 取舍记档） | ⛔ 2026-09-08 取消 |
| 7.4 文件搜索分发定案 | 决策：`dd-ext-search` 保持 sidecar 或纳入内嵌物化机制（`build.rs` EMBED_EXES 表扩第 6 个成员） | 决策记档 + 实施（或明确延后理由） | ✅ 2026-09-08 定案：sidecar，随绿色包 `dist/extensions.d/` 携带（`package.sh` 归集；理由见 M7 取舍记档） |
| 7.5 发布流程 | tag → `package.sh` 产物 + GitHub Release 附着（dist 单文件 + sidecar 目录 zip）；**便携 sidecar 扫描落地**（宿主扫描 exe 同目录 `extensions.d/`，绿色包解压即用） | code：fmt/clippy/全 workspace 测试绿（+2 sidecar 合并单测）+ zip 布局本机验证；**tag 演练 ✅（v0.1.1 已发布 2026-09-10）；仅剩真机解压即用走查** | 🟨 代码完成（2026-09-08），tag 演练完成 |

已做取舍记档（7.1）：CI 测试显式跳过 `apps::steam_installed_shown_uninstalled_filtered_root_lnk_shown`——该测试硬编码期望本机已装应用「Flowframes」，机器相关（2026-09-07 记录），CI 环境无法满足；后续批次改为机器无关夹具后移除跳过。已做取舍记档（7.2）：256 档沿用 BMP 条目（与既有五档同格式，无新增消费方差异；PNG 压缩条目仅省体积，`.ico` 总尺寸非当前瓶颈）。已做取舍记档（7.3/7.4）：① **安装器按用户决策取消（2026-09-08）**——分发形态定案为**仅免安装单文件绿色版**（`dist/dd-run-<版本>.exe` + `extensions.d/` sidecar 目录，zip 附着 Release）；Inno Setup 方案曾完整落地（按用户安装 `{localappdata}\Programs\dd-run`、自启任务、file-search 清单写入、卸载清理）并通过静默安装+卸载真机冒烟，代码随决策整体移除，安装器相关文件不入库；② **file-search sidecar 定案（7.4 关账）**：不纳入内嵌物化——文件搜索依赖 Everything 用户环境，纳入内嵌会让单文件宿主强行携带非必需依赖，sidecar 让「带不带文件搜索」与宿主本体解耦，且 EMBEDDED 表扩成员需同步 `dd-host::BUILTINS` 改动成本高；sidecar 经 `package.sh` 归集到 `dist/extensions.d/`（清单 `${EXT_DIR}` 解析到本目录）；③ 安装器过程踩坑（Inno `DestName` 仅 external 条目有效、staging 通配须排除 `setup-` 产物、`CurStepChanged` 参数是 `TSetupStep` 前缀 `ss`）随安装器取消不再适用，仅记档备查。已做取舍记档（7.5）：**便携 sidecar 扫描（本批关键缺口修复）**——免安装 zip 布局 `dd-run-<版本>.exe + extensions.d/` 解压后，宿主原只扫描用户数据目录 `%APPDATA%\dd-run\extensions.d`，exe 旁 sidecar **不会被自动发现**、「解压即用」不成立（7.5 演练暴露）；修复 = `aggregator.rs::load_extension_sources` 增**宿主 exe 同目录 `extensions.d/` 扫描**（`sidecar_extensions_dir`），两处扫描按 id 去重：**用户目录优先**（用户手动放置版本覆盖分发版）、sidecar 独有追加，内置扩展仍最优先（`merge_builtins`）；`manifest-schema.md` §2 增补该位置（便携应用惯例 sidecar 跟随 exe）；sidecar 清单 `${EXT_DIR}` 解析为该 sidecar 目录（文件搜索已按此路径展开）。
...
按 2026-09-05 既定路线的第三站：M6「日常好用」与 M7「发布工程」已关账，本里程碑验证**「协议与语言无关」不是一句声明**——用**非 Rust** 扩展走通协议全链路。

立项时的三项缺口：① 仓库只有规范（`protocol.md` / `manifest-schema.md`），**没有面向人的上手路径**；② 唯一示例 `crates/dd-ext-sample` 是 624 行 Rust，**不是最小 hello**，更没有非 Rust 实证；③ 自检工具 `dd-run-cli` 止步于 `top_level_commands`，**覆盖不到嵌套页 / 兜底 / `host/*`** —— 而这三处恰是第三方最容易踩坑的地方。

| 批次 | 内容 | 验收标准 | 状态 |
|---|---|---|---|
| 8.1 扩展开发指南 | 新增 [`docs/extensions.md`](./extensions.md)：30 秒心智模型 / 10 分钟上手 / 三条铁律 / 三个 Windows 陷阱 / 握手 / 各方法按「值得实现」排序 / `host/*` 反向请求 / 自检清单 / 调试手册（症状→原因对照）/ 崩溃语义 / 超时预算 | 一个没读过本仓库源码的人能照着写出可被宿主加载的扩展 | ✅ 2026-09-10 |
| 8.2 非 Rust 全表面示例 | 新增 [`examples/python-minimal/`](../examples/python-minimal/)（仅标准库）：覆盖 7 个 host→ext 方法 + `items_changed` + 3 种 `host/*` 请求 + `ShowToast`/`Dismiss`/`Confirm`；附 Windows `.cmd` 启动器与清单 | `dd-run-cli --conformance` 全绿 | ✅ 2026-09-10 |
| 8.3 `--conformance` 全表面自检 | 自检从 4 步扩到**全表面**：`fallback_commands` 一致性（非空 ⟺ `has_fallback`）/ `get_command` 可复热性 / `get_items` 结构 / `invoke` 返回值 ∈ §8.3 八种 / `host/*` 声明一致性 / `close`；逐项 `✓ ⚠ ✗`，任一 ✗ 退出码非 0；新增 `--invoke`（默认跳过真实副作用）与 `--ext-id` | 对 8.2 的示例 10 项全 ✓ | ✅ 2026-09-10 |
| 8.4 端到端实证 | `dd-run-cli --conformance --extensions-dir examples/python-minimal --invoke` | **10 项全 ✓（2386 ms）** —— 非 Rust 扩展走通协议全链路 | ✅ 2026-09-10 |

已做取舍记档（8.1）：指南**只写规范里没有的东西**——路径、陷阱、自检、排错；凡是与 `protocol.md` / `manifest-schema.md` 重复的一律改为引用，并显式声明「冲突以规范为准」。已做取舍记档（8.2）：① 只给 **Python 单语言**（先把链路证实、成本最低；Node 版按需再加）；② 做**全表面**而非最小骨架（示例的价值在于「抄完就能用」）；③ 启动器用 **`.cmd` 桥接**——清单的 `entry.args` **不支持** `${EXT_DIR}` 展开（`manifest-schema` §4 只覆盖 `command`/`cwd`/`icon`），无法表达「解释器 + 脚本」两段式命令行；实测宿主 `resolve_executable` 会为无扩展名路径补 `.cmd`，且 Rust `Command` 能直接 spawn `.cmd`（本批实测探针：spawn 27 ms、initialize 1459 ms）。已做取舍记档（8.3）：`invoke` **默认跳过**——它会真的产生副作用（写剪贴板 / 开浏览器 / 关机），要跑必须显式 `--invoke`；自检刻意用**原始 JSON** 而非强类型反序列化（§6/§8 规定的是**线上形状**，强类型反序列化成功反而掩盖「多字段 / 错类型」问题）。

**真机反馈修复（2026-09-10，同日）**：用户把示例装进 `dist/extensions.d/` 后，扩展管理能列出 PyMin 但标为**暂时不可用**、点「重试」无效。① **根因**：`dd-ext-pymin.cmd` 调用**裸 `python`**，依赖解释器在 PATH 上；本机 `where python` / `where py` **均无输出**（Python 只在 IDE 管理的目录里），而 **GUI 从资源管理器启动继承系统 PATH** → cmd 报「'python' 不是内部或外部命令」→ spawn 失败 → 连续 3 次熔断。**自检全绿是因为它从终端启动、PATH 里恰好有解释器**。② **修复**：示例改为**安装脚本生成清单**（`examples/python-minimal/install.py`）——把 `sys.executable` 与脚本的**绝对路径**写入 `entry.command` / `entry.args`（宿主直接 spawn、**不经 shell → 零 PATH 依赖**）；移除示例目录的静态清单（固定清单对解释型扩展写不对，且正是假阳性来源）；`.cmd` 降级为**备选**并在文件头写明前提。③ **指南同步**：`extensions.md` §1 第 4 步改为推荐绝对路径清单；§3 由「两个 Windows 陷阱」扩为**三个**（新增「解释器不在 PATH 上」）；§8 症状表补 2 行。④ **验证**：把 PATH 摘到只剩 `System32`（**完全无 python**）后 `dd-run-cli --conformance --ext-id com.example.pymin --invoke` → **10 项全 ✓（902 ms）**。⑤ **暴露的工具盲区（已写入指南）**：`--conformance` 继承**终端**环境，**覆盖不了 GUI 的环境差异**——「自检绿」不等于「GUI 能启动」。
...
| R4 | 上游 PowerToys 文档引用边界复核（许可证本身已定） | 已采用 **MIT**（根 `LICENSE` + 各 crate `license = "MIT"`），与 README 声明一致；✅ 引用边界已复核销项（批次 8.3，2026-09-10，结论见 §6.1 L6） |
...
| L2 | 熔断后手动重试入口 | m4-record §5 | ✅ 已销项（M6 批次 6.4，commit `a007656`）：设置页「扩展管理」卡对 Failed（熔断）扩展显示「重试」按钮 → `reset_crash` + `restart_aggregation`（`settings_view.rs` + `health.rs` 单测 `reset_crash_clears_trip`）；真机验收待做（连续崩溃熔断 → 重试恢复，并入 M6 集中回归） |
| L3 | 顶层 `items_changed` 不重聚合 | m3-record §5 / m4-record §5 | ✅ 已销项（M6 批次 6.4，commit `a007656`）：`RefreshState.top` 标记 + `tick_refresh` 顶层分支调 `restart_aggregation` 全量重聚合（`refresh.rs` 单测 ×2）；真机验收待做（顶层 items_changed → Root 刷新，并入 M6 集中回归） |
...
| L5 | LRU 驱逐后 fallback 复热失败 | m4-record §3.6 / §5 | ✅ 已销项（方向 C 批次，2026-09-10）：根因 = 协议 §6.4 `get_command` **只查顶层命令**（`dd-ext/src/lib.rs`），兜底模板 id（如 `calc.eval.query`）天然不在顶层注册表 → 桩复热拿 `Ok(None)` 即判「命令已失效」并短路（`invoke` 从未发出）。修复 = `FallbackStore::contains_template(ext_id, id)` + invoke 侧 `skip_lookup` / page 侧 `command_id=None` 跳过校验直执行（**常规命令保持 §6.4 陈旧校验不变**）。与同批「warm 空闲回收」耦合——回收让驱逐真实发生，该缺陷由"潜伏"变"必现"，故同批修复。真机验收：驱逐后点兜底项应正常执行 |
| L6 | 上游 PowerToys 引用边界复核 | §6 R4 | ✅ 已销项（批次 8.3，2026-09-10）：唯一派生文档 `cmdpal-platform-agnostic-design.md` 已明确标注为「参考设计」、含核验基准 `v0.101.2362.0` + 事实来源 + 「不宣称语言无关」声明；设计稿仅 3 处「PowerToys Run 同款」对比性灵感标注、无抄录；代码为原创 Rust、协议 v1.0 独立冻结；MIT 许可证已定（根 LICENSE + 各 crate）。引用边界合规，无需进一步动作 |
...
| L9 | IME 交互中文输入环境人工复验 | 2026-09-03 记录 | ✅ 已销项（2026-09-08，M6 集中回归 D 组真机复验通过：中文输入候选框位置正确、组合输入正常——SendInput 无法模拟故人工确认） |
...
**当前状态（2026-09-06 15:58）**：**真机反馈两连修——① 面板基准高度 +20（420→440）；② 「拉伸后重开仍默认」复现定性 + 去抖落盘补强**。② 取证：自动化复现脚本（SendInput 拖右缘 + Alt+Space 唤起 + Esc，`.workbuddy/tmp/test_resize_persist.py`）在 15:12 版 exe 上**端到端复现通过**（落盘日志 + config `panel_size` + 再唤起保持 890 宽）——用户观察到的失效最可能是拉伸发生在旧版 exe（无落盘）会话中，新 exe 无记忆可恢复；另发现隐患 = 强杀进程不走 hide()/托盘退出会丢档（原设计仅在隐藏/退出落盘）。补强：`app/mod.rs::ui()` 新增**可见期间去抖落盘**——尺寸偏离 `shown_size` 持续 ≥500ms 即写盘（拖拽缩放期间 BeginResize 原生模态循环无 egui 帧，松手后才开始计时，天然去抖），强杀最多丢最后 500ms；`PaletteApp` 增 `resize_pending_since` 字段。① `APP_H` 420→440（内容区 440−52−36=352 → 仍 8 行@40）；设计稿 v4.12 全量同步（650×420 → 650×440 共 12 处：CSS/00.1/D28 注记/D37/偏离清单/00.4/01 几何/8.1/G1/版本履历）。测试教训：尺寸单测曾写死 420 字面值导致 2 例假失败（cargo test 目标失败即中止后续目标，全量 passed 从 253 变 189——遇 passed 总数骤降先查 fail-fast 中止）。自动化验证（15:58 版 exe）：唤起 650×440 ✅ / 拖拽 866→890 过程中可见状态即落盘 ✅ / 再唤起保持 890×440 ✅；测试后已清 config 残留并杀进程。`fmt --check` ✅ / clippy 0 告警 / **253 passed / 0 failed**；release BUILD_EXIT=0，`dist/dd-run.exe`（10.5M，15:58）。**未提交**。

**当前状态（2026-09-06 15:28）**：**真机反馈修复——应用名缺字方块（AMD Software□ Adrenalin Edition）**。取证：开始菜单 .lnk 实际文件名 = `AMD Software꞉ Adrenalin Edition.lnk`，其中冒号为 **U+A78B（MODIFIER LETTER COLON「꞉」**，AMD 用它规避 Windows 文件名非法字符 `:`）；fonttools 核验 cmap：msyh.ttc（29905 码位）与 seguisym.ttf（7536）均**无** U+A78B，`segoeui.ttf`（3996）**有**（开始菜单即用 Segoe UI 渲染正常）→ 根因 = 字体栈缺字形，非名称提取损坏（`SIGDN_NORMALDISPLAY` → `to_string_lossy` 链路无损）。修复：`platform.rs::load_cjk_font_definitions` 字体栈插入 **segoeui.ttf**（Segoe UI，Vista+ 必装）作拉丁/符号扩展后援，顺序 cjk → sym → **segoe** → icons；segoe cmap **零 PUA 码位**（已核验 U+E000–U+F8FF 为空），不会抢 §8.6 图标字形；读盘在既有后台字体线程（+~1.1MB，不在主路径）。已知边界（记档）：搜索仍按原字符匹配——输入 ASCII `:` 不会命中 `꞉`（按名搜索「AMD Software」即可）。`fmt --check` ✅ / clippy **0 告警** / dd-gui 134 测试全绿；release 重编 BUILD_EXIT=0，`dist/dd-run.exe` 覆盖（10.5M，15:28，Stop-Process 终止运行中的旧实例）。**真机验收**：唤起面板搜「AMD」→ 名字显示「AMD Software꞉ Adrenalin Edition」无方块。

**当前状态（2026-09-06 15:12）**：**批次 4.6 面板尺寸自适应与记忆（设计稿 v4.12 D37）代码完成**。动机：①「拉伸面板后重新打开恢复默认宽高」——根因非 bug，是 D36④「尺寸不落盘」的 grill 决策，本批按用户决策推翻该半条（D36 其余语义保留）；② 基准尺寸调整 560×460→650×420、设置页 640×640→650×640（宽度与根页对齐）。实施（4 文件）：`settings.rs` 新增 `panel_size: Option<(u32,u32)>`（手工 serde_json `[w,h]`/null，缺失/损坏/越界→None，取整存储避免 f32 破坏 `Eq`）；`app/mod.rs` 常量 APP_W/H=650/420、SETTINGS_W/H=650/640 + 纯函数 `clamp_to_workarea`/`root_panel_size`/`settings_panel_size`（等比收缩 + `[460×400, 工作区−16]` clamp，下限恒成立；设置页 = max(650×640, 根页有效尺寸) 逐轴取大）；`platform.rs::send_center_on_cursor` 返回光标屏工作区（逻辑点）且居中改用窗口**当前实际**逻辑尺寸；`app/lifecycle.rs` show() 按工作区/记忆值动态计算唤起尺寸（`shown_size` 记录程序设定值），hide()/托盘 Exit 经 `persist_panel_size` 落盘——仅当前尺寸偏离 `shown_size`（用户真拉伸）才写盘，防止小屏 clamp 的程序尺寸误存；`ui()` settings_sized diff 收口点改动态计算。`fmt/clippy/test` 全绿（**253 passed / 0 failed**，+7：尺寸纯函数 6 + panel_size 序列化 1）；release 重编 BUILD_EXIT=0，`dist/dd-run.exe` 覆盖（10.5M，15:12）。**真机验收待做（设计稿 G1–G5）**：唤起自适应/拉伸记忆含重启/clamp 边界/设置页联动/全区间弹性不破版。

**当前状态（2026-09-09 20:xx）**：**文件搜索 v3.3 P0/P1 落地**（§九 变更方案，协议 v1.0 零改动）。P0：`files.hint/guide/error` 占位项点击专属 Toast（`7e427e7` 已含）+ `guide_item` subtitle 与模块头注释对齐 es.exe 通道（移除过时 HTTP 表述）。P1（more_commands 三动作，对齐 ECP 高频操作）：① 扩展侧 `search.rs`——文件结果 `more_commands = [files.reveal.{pid}, files.copy.{pid}]`（三动作共享 PATH_INDEX pid）；`resolve_path_action` 纯决策严格校验（前缀/pid 数字/在索引中/`context.selected_item_id` 冗余一致），失效回「路径已失效」Toast 零副作用；reveal 走 `CommandExt::raw_arg` 构造 `explorer /select,"<path>"`（引号坑规避，非 Windows 回退 xdg-open 父目录）；copy 回 ShowToast + `Effect::HostRequest(host/set_clipboard)`（运行时保证响应先于副作用）；`spec().capabilities` 追加 `host/set_clipboard`。② 宿主侧调用链（P0 第 3 步前置缺口补齐）——`PanelItem.more_commands` 透传（`to_panel_item`），`ctx_menu` 渲染扩展动作（非空时 GUI 静态类别映射让位，防与静态动作重复），激活以 `sender=context_menu` + `selected_item_id` 回调 `invoke`（`result::context_menu_invoke_params`，复用 warm/复热/熔断分派）；能力前置由 dd-host 既有 `-32601` 机制兑现。③ manifest `examples/extensions.d/com.ddrun.filesearch.json` capabilities 同步。验证：dd-ext 单测 +5（三动作构造/校验/copy 副作用/失效拒绝/spec-manifest 一致性 `include_str!`）、dd-gui 单测 +4（ctx_menu 渲染 2 + result 参数 1 + aggregator 透传 1）；`everything_unavailable_returns_guide` 改可用性注入版（`get_file_items_with`，§9.3 L1 单测不依赖 Everything——本机 Everything 在跑时旧版会真搜出结果）；L2 NDJSON 冒烟全过（`.workbuddy/tmp/smoke_p1.py`：initialize capabilities / get_items more_commands / copy Toast+剪贴板请求+宿主应答 / 失效 pid 零副作用）；fmt/clippy 零告警、workspace 301 passed。**真机验收待做**：右键三动作（含空格/中文路径 reveal 引号正确性）、copy 后剪贴板实际内容、失效项 Toast。P2（everything-ipc 直连）待实施，硬前置 = 先核 docs.rs `RequestFlags`/`DateModified`。
...
| 4.6 ✅ | 面板尺寸自适应与记忆（设计稿 v4.12 D37，推翻 D36④ 半条） | ① 基准尺寸 560×460→**650×420**、设置页 640×640→**650×640**（宽度与根页对齐）；② **唤起自适应**——`send_center_on_cursor` 改返回光标屏工作区（逻辑点），`show()` 按 `root_panel_size(工作区, 记忆值)` 动态计算唤起尺寸：工作区充裕用基准/记忆值，不足等比收缩，clamp 到 `[460×400, 工作区−16]`（下限恒成立）；③ **拉伸记忆**——`settings.rs` 新增 `panel_size: Option<(u32,u32)>`（手工 serde_json `[w,h]`/null），`hide()`/托盘 Exit 经 `persist_panel_size` 落盘：仅当前尺寸偏离 `show()` 程序设定值 `shown_size`（= 用户真拉伸）才写盘，等于基准存 `None`；④ **设置页联动**——`ui()` settings_sized diff 收口点改 `settings_panel_size` 动态计算 = max(650×640, 根页有效尺寸) 逐轴取大再 clamp；⑤ 居中改用窗口当前实际逻辑尺寸（记忆拉伸后 ≠ 基准常量）。新增纯函数 `clamp_to_workarea`/`root_panel_size`/`settings_panel_size`（`app/mod.rs`）。 | 设计稿验收 **G1–G5**（唤起自适应 / 拉伸记忆含重启 / clamp 边界 / 设置页联动 / 全区间弹性不破版）；`fmt/clippy/test` 全绿（**253 passed**，+7）；release 重编 BUILD_EXIT=0，`dist/dd-run.exe` 覆盖。**真机验收待做**。 |
...
**当前状态（2026-09-08，M7 立项 + 批次 7.1/7.2，已 commit `a789906`）**：**进入 M7 发布工程**（2026-09-05 既定路线，M6 代码全部完成只剩真机验收）。① **文档纠偏**：§6.1 台账 L2/L3 实际已于 M6 批次 6.4（`a007656`）销项（设置页扩展管理「重试」按钮 + 顶层 `items_changed` 重聚合，代码/单测均已在库），台账两行补 ✅；§5 表 M6 行同步为「代码全部完成，剩余真机验收」。② **批次 7.1 CI**：新增 `.github/workflows/ci.yml`——windows-latest + `stable-x86_64-pc-windows-gnu` 工具链（仓库 `.cargo/config.toml` 默认 target 即 gnu，self-contained 链接器 rustup 自带），四关：build / test（`--skip steam_installed_shown_uninstalled_filtered_root_lnk_shown`，机器相关硬编码期望，见 M7 取舍记档）/ clippy `-D warnings` / fmt `--check`；`CARGO_INCREMENTAL=0` + rust-cache。**CI 首跑绿灯（2026-09-08，run #1，commit `a789906`，耗时 4m23s）→ 批次 7.1 关账**。③ **批次 7.2 256px 图标**：`tools/gen_icon.py` SIZES 增 256（LANCZOS、BMP 条目），重新生成 `assets/app.ico`（六档 16/20/24/32/48/256）；`tray.rs` ICO 容器守卫测试 `embedded_ico_has_dpi_tier_entries` 5→6 档同步。**验证**：本地 fmt / clippy / `cargo test -p dd-gui` 全绿。**附带工程卫生**：① `cargo fmt --check` 暴露上轮遗留格式漂移（search.rs / dd-ext lib.rs / theme.rs / panel.rs / roundtrip_builtins.rs 等）→ `cargo fmt --all` 统一，复验 0 diff；② **发现并修复 M6 i18n 批次引入的确定性测试回归**——`roundtrip_builtins::builtin_initialize_matches_registry` 断言扩展握手自述 `display_name` 与注册表英文名一致，但 M6 i18n（批次 D）后扩展自述名经 `tr()` 本地化、缺 `DDRUN_LANG` 时无条件回落 zh_cn → 必然失配（上轮"全绿"结论漏跑此断言）；修复 = 测试 `load_builtin` helper 经既有 `entry.env` 通道注入 `DDRUN_LANG=en_us`（注册表名是宿主侧英文常量，真机中文显示本地化名属预期，语义不同记档于测试注释）；③ 复跑全 workspace 测试（CI 同口径，skip steam 机器相关测试）**276 通过 / 0 失败** + clippy `-D warnings` 0 + fmt 0。**M6 集中回归清单（真机，用户执行）**：A) 6.3 设置三项——热键重绑全局生效 + 冲突还原、开机自启注册表 Run 键落地/删除、扩展启停即时生效且重启保留；B) L8——嵌套页顶行统一 / Loading 骨架 / Dialog 遮罩+Toast（A1–A5/C1–C3）；C) L2/L3——熔断后「重试」按钮恢复、顶层 items_changed Root 刷新；D) L9 IME 中文输入人工复验；E) v4.17a 亚克力面板样式/光标/选中 + 拖动缩放后不空白；F) 托盘七项（M6 立项记录 ①–⑦）。**已 commit + push `a789906`**（12 文件：ci.yml 新建 + gen_icon.py/app.ico/tray.rs + roundtrip_builtins.rs 修复 + fmt 5 文件 + implementation.md / 2×README），CI 首跑绿灯。

**当前状态（2026-09-08 补，CI 首跑验证）**：GitHub Actions run #1（commit `a789906`）✅ 绿，4m23s，四关（build/test/clippy/fmt）全过 → **批次 7.1 正式关账**；下一步 7.3 Inno Setup 安装器。

**当前状态（2026-09-08，M7 批次 7.3 安装器 + 7.4 定案；⚠️ 同日按用户决策取消安装器，见下一条）**：**Inno Setup 安装器落地 + file-search 分发定案 sidecar**。① **新增 `tools/installer.iss`**（Inno Setup 6）：`PrivilegesRequired=lowest` 按用户安装到 `{localappdata}\Programs\dd-run`；文件 = 单文件宿主 `dd-run.exe` + `extensions.d/dd-ext-search.exe`（ignoreversion 全量覆盖，升级即重装）；开始菜单快捷方式（可选桌面快捷方式任务）；可选开机自启任务写 HKCU Run 键（键名/值格式与 `platform.rs::set_autostart` 完全一致 + `uninsdeletevalue` 卸载清理）；`[Code]` `CurStepChanged(ssPostInstall)` 把 file-search 清单写入 `%APPDATA%\dd-run\extensions.d\com.ddrun.filesearch.json`，`entry.command` 经 ISPP 展开为**正斜杠绝对路径**指向安装目录（`manifest.rs` 的 `is_relative` 认 `C:/` 形态为绝对路径、`.exe` 后缀 `resolve_executable` 自动补全）；卸载必清安装目录/快捷方式/自启值/安装器写入的清单（死清单），用户数据 `%APPDATA%\dd-run` 卸载收尾询问后可选删。② **新增 `tools/build_installer.cmd`**：自动探测 ISCC（机器 x86/x64/用户级三位置，缺失时提示 winget 安装命令）→ 复制 `dist/dd-run-<版本>.exe` 为固定名 `dist/staging/dd-run.exe` → 编译出 `dist/dd-run-setup-<版本>.exe`。**踩坑记档**：Inno `DestName` 仅对 `external` 标志条目有效，普通条目按源文件名落盘（首版冒烟实测装出 `dd-run-0.1.0.exe`）→ 用 staging 固定名文件规避。③ **7.4 提前定案**：file-search 保持 sidecar（依赖 Everything 用户环境 + 与宿主本体解耦 + EMBEDDED/BUILTINS 同步成本），理由记档 M7 取舍记档 ②。**验证（本机真机冒烟，Inno Setup 6.7.3）**：静默安装 → `{app}` 下 `dd-run.exe`/`extensions.d/dd-ext-search.exe`/`unins000.exe` 就位 + 开始菜单快捷方式 + 清单 command 重写为安装目录绝对路径（内容核验通过）→ 静默卸载 → 安装目录/清单全清、既有的用户自启 Run 值（指向别处副本）原样保留未误删；本机 Run 键已有用户手工自启项，安装器未勾选任务时不写不删（沉默复用语义正确）。**留用户**：交互式向导走查（选目录/任务勾选/卸载询问保留数据）+ 升级场景（旧版本在装时重装覆盖）。

**当前状态（2026-09-08 补，安装器取消 → 仅免安装分发）**：**用户决策「清理安装器相关代码，只提供免安装版本」**。① **删除** `tools/installer.iss` 与 `tools/build_installer.cmd`（未入库，直接移除）；`tools/package.sh` 移除安装器编译段（原 [6/6]），保留并保留命名 file-search sidecar 归集段（现为 [5/5]，注释改为「随绿色包 dist/extensions.d/ 携带」）——7.4 sidecar 定案不受取消影响，只改携带方式（安装器 → 绿色包目录）；`dist/` 下的 staging 目录与 `dd-run-setup-*.exe` 本地产物一并清理。② 分发形态终态 = **免安装绿色版**：`dist/dd-run-<版本>.exe` 单文件（5 内置扩展内嵌）+ `dist/extensions.d/`（file-search sidecar），发版时整体 zip 附着 GitHub Release（批次 7.5 验收口径不变）；开机自启继续走应用内设置页（HKCU Run 键），不提供安装器代配。③ 台账与取舍记档同步（M7 表 7.3 ⛔ 取消 / 7.4 ✅ sidecar、取舍记档 7.3/7.4 重写），README 双语 M7 行同步。**最终改动集**：`tools/package.sh`（sidecar 段新增 + 安装器段不引入）+ `docs/implementation.md` + 双 README，安装器两文件不入库。

**当前状态（2026-09-08，M7 批次 7.5 便携 sidecar 扫描，commit `818d1db`）**：**「解压即用」关键缺口修复 + zip 布局本机演练**。① **缺口暴露**：7.5 演练按定案布局打 zip（`dd-run-<版本>.exe` + `extensions.d/`），但宿主 `load_extension_sources` 原只扫描用户数据目录 `%APPDATA%\dd-run\extensions.d`（manifest-schema §2 仅一表），exe 旁 sidecar **不被扫描**——「解压即用」不成立（清单在包里，宿主在别处找）。② **修复（`aggregator.rs`）**：`load_extension_sources` 新增**宿主 exe 同目录 `extensions.d/` 便携扫描**——`sidecar_extensions_dir()`（`current_exe.parent()`/`extensions.d`）+ `merge_sidecar_scan()`/`merge_scanned_dirs()`（两处按 id 去重：**用户目录优先**覆盖分发版，sidecar 独有追加；内置仍最优先）；`push_note` 复用于 note 拼接，目录缺失经 `scan_dir` 视作空（非错误），开发期/内嵌物化目录无 sidecar 时零影响。③ **规范同步**：`manifest-schema.md` §2 增「便携 sidecar（v1.0 补充，M7 批次 7.5）」——位置（exe 同目录）、优先级、`${EXT_DIR}` 语义。④ **测试**：aggregator 单测 +2——`merge_scanned_dirs_user_dir_wins_and_sidecar_appends`（同 id 去重 + sidecar 追加顺序 + 用户版保留）/ `merge_scanned_dirs_empty_sidecar_keeps_user_order`；`LoadedExtension` 可构造（manifest.rs 全字段公开）。**验证**：fmt 干净、clippy `-D warnings` 0、dd-gui lib **143 通过 / 0 失败**、全 workspace（CI 同口径 skip steam）**全绿 / 0 失败**。**release.yml 已备**（上轮免安装定案时写入）：tag → `package.sh` → `Compress-Archive` 打 `dist/dd-run-<版本>-portable.zip`（exe + extensions.d）→ `action-gh-release` 附着。**剩余（真机）**：解压 zip 到无扩展环境 → 直接运行确认 file-search 出现（页脚无「扩展目录不可读」、`f ` 前缀可进）、升级覆盖；tag 演练发版一次关账 7.5；M6 集中回归清单照旧。（本批代码已随 commit `818d1db` 入库。）
**当前状态（2026-09-08，M6 集中回归清单验收完成）**：**用户真机执行 M6 集中回归清单 A–F 六组全部验证通过 → M6 正式关账**。A) 6.3 设置三项——热键重绑全局生效 + 冲突还原、开机自启注册表 Run 键落地/删除、扩展启停即时生效且重启保留 ✅；B) L8——嵌套页顶行统一 / Loading 骨架 / Dialog 遮罩+Toast（A1–A5/C1–C3）✅；C) L2/L3——熔断后「重试」按钮恢复、顶层 items_changed Root 刷新 ✅；D) L9 IME 中文输入人工复验 ✅（台账销项）；E) v4.17a——亚克力面板样式/光标/选中 + 拖动缩放后不空白 ✅；F) 托盘七项（M6 立项记录 ①–⑦）✅。**同步**：§5 M6 行 → ✅ 已关闭；§6 批次表 6.3「真机验收待做」→「真机验收通过」、6.4 行 L1/L9 销项；§6.1 台账 L9 销项；§5 M7 行状态列校正（7.3 ⛔ / 7.4 ✅ / 7.5 🟨，与 M7 表一致）；README 双语 M6 行 → complete。至此遗留台账仅剩 **L5**（LRU 复热 fallback，第三方多扩展场景，内置 5 < LRU 8 不触发）/ **L6**（引用边界复核）两项开放，均非阻断；M7 批次 7.5 剩余真机走查（解压即用 + tag 演练）独立推进。

**当前状态（2026-09-10，批次 8.1：`file://` URL 解析三缺陷修复，未 commit）**：**阶段 1 收口批次 1（`docs/search-file.md` §9.6 三项确定缺陷）全部修复**。按上轮方向评审的用户决策「阶段 2 主线 = 先只做阶段 1 收口」，本批取三缺陷中**确定性最高**的一项先行（另两项 A-33 真机验收 / 7.5 解压走查依赖真机，不阻塞本批）。① **方案取舍**：宿主侧「存在性优先 + decode 兜底」vs 扩展侧 percent-encode → **采用宿主侧**（理由：宿主侧已覆盖「文件名含 `%`」「标准编码 URL」两种来源；而 file-search 是 sidecar，改扩展侧会让扩展与宿主版本产生隐性耦合；`search.rs` 侧只补 UNC 识别，不做 encode）。② **宿主侧**（`platform.rs`）：删 `file_url_to_path`（无条件 decode），新增 `split_file_url`（拆 authority）/ `to_windows_path`（有 host 拼 UNC）/ `strip_query_fragment`（去 `?query`/`#fragment`）/ `file_url_candidates`（纯函数，排序候选 = 原样 → 去 query/fragment → 上述两者的 decode）/ `resolve_file_url_to_path`（取第一个存在者；**UNC 不做 `exists()`**——离线 SMB 共享会阻塞到网络超时，直接交 `ShellExecuteW`）；调用点 `app/host_actions.rs` 改用新函数。**原 5 条 `file_url_to_path_*` 单测改写为候选列表口径 + 新增 3 条**（含真实文件系统夹具：同目录并存 `report%20final.txt` 与 `report final.txt`，断言前者优先；删前者后回退后者）。③ **扩展侧**（`search.rs`）：新增 `path_to_file_url`——UNC（`\\server\share\x`）→ `file://server/share/x`（此前拼成 `file:////server/…` 必然打不开），常规路径行为不变；新增 4 单测。**验证**：`cargo fmt --all --check` 干净、`clippy --workspace --all-targets -- -D warnings` 0 告警、`cargo test --workspace`（CI 同口径 skip steam 机器相关测试）**324 passed / 0 failed**。**遗留真机**：A-33-03 用例补 `%` / `#` 的实际打开验证（随 A-33 其余四项一并执行，属批次 8.2）。**环境洼地记录（重要）**：本机 Bash 会话 `APPDATA` 为空（`USERPROFILE`/`TEMP` 正常），`cache_dir()` 依赖它 → 未导出时 `dd-ext-apps` 图标抽取两条测试**必失败**（覆盖率 0%，PNG 无法落盘）；补 `export APPDATA="C:\Users\<user>\AppData\Roaming"` 后 12/12 全过。**今后跑测试先看一眼 APPDATA，别把环境缺失误判成代码回归**（已同步 CHANGELOG）。同步改动：`CHANGELOG.md`（新增「文件搜索 v3.3 P2：`file://` URL 解析三缺陷」修复条目）、`docs/search-file.md` §9.6（三项标 ✅ 已修 + 新增 §9.6.1 实施记录含取舍理由）。协议 v1.0 零改动。

**当前状态（2026-09-10，批次 8.3：L6 引用边界复核销项）**：**§6.1 台账 L6 关闭**。复核范围 = 全库对上游 `microsoft/PowerToys`（CmdPal）的引用。结论：① 唯一派生文档 `cmdpal-platform-agnostic-design.md` 自身已明确声明为「从 PowerToys `src/modules/cmdpal` 提炼的**参考设计**」，含核验基准 `v0.101.2362.0`、事实来源（各 doc 路径）、「本文不宣称语言无关」免责声明，整篇带归因——属合规的派生/参考实践；② 设计稿 `cmdpal-ui-mockups.html` 仅 3 处「PowerToys Run / ueli 同款」「PowerToys Run 同款做法」「PowerToys Run 行为」的**对比性灵感标注**，无抄录代码或文案；③ 宿主/扩展源码为原创 Rust（协议 v1.0 独立冻结，未复用 PowerToys C#/WinRT 实现）；④ MIT 许可证已定（根 `LICENSE` + 各 crate `license = "MIT"`）。四项均满足「引用边界清晰、归因到位、无未授权复制」，L6 销项；R4 同步关闭。台账开放项至此仅剩 **L5**（第三方多扩展 LRU 复热 fallback，属阶段 2 扩展生态的天然触发点，本轮不动）。

**当前状态（2026-09-09，全库文档核对修正轮）**：用户指令「重新阅读代码与文档，整理核对」，确认范围 = A 类事实错误全改 + B 类漂移全改 + C1 发版风险处理。**A1/A2**：README 双语「构建与打包」整节与 Highlights 分发行按 7.4/7.5 定案重写——旧文「无 sidecar 文件 / 不做多文件 zip」与现行绿色 zip（exe + `extensions.d/`）直接矛盾；**A3**：双语文档导航补 `docs/search.md`、implementation.md 描述 M0–M6→M0–M7；**A4**：manifest-schema.md §10 重写——「内置扩展也走清单、由安装器写入」的 MVP 设想按现行实现更正（`BUILTINS` 代码注册表 + 编译期内嵌物化 + `merge_builtins` 最优先；file-search = 唯一清单型官方扩展 sidecar；优先级：内置 > 用户目录 > sidecar）；**A5**：protocol.md §14 注记 `id:10`→`id:1` 对齐附录示例，并补「两侧 id 空间独立」说明（已核验 `consistency.rs` 只解析 `### N.M` 下的 ```json 围栏，§14 无测试耦合；`cargo test -p dd-protocol` 全绿）；**B1**：README 双语设计稿引用 v4.17→v4.13（HTML 文件实况，最后更新停在 `3228b0a`），M6 行注明 v4.14–v4.17a 批次未回同步 HTML；**B2**：§7 7.5 条目两处「未 commit」→ 已随 `818d1db` 入库；**B3**：CHANGELOG 传输层标题补「es.exe 切换（`6133d96`）晚于 v0.1.0 标签、该标签从未触发发版」；**C1（风险，待用户定版本策略）**：tag `v0.1.0` → `3228b0a` 的树中**无** release.yml（`git cat-file -e` exit 128）且 search.rs 为 HTTP 旧实现，GitHub 实抓无任何 Release——直接推旧 tag 不会触发发版工作流、发出去也是旧搜索实现，选项 = 删除重打到 HEAD / bump 0.1.1 / 保留现状仅演练。**已决策并执行（选项 1，2026-09-09）**：本地与远端 `v0.1.0` 删除后重打至含本轮修正的提交并推送——触发 release.yml 首次发版演练（重打后 tag 树含 release.yml 与 es.exe 搜索实现，CHANGELOG 传输层标题同步该口径）。**核对澄清的非问题**：🪟 计数——设计文档 §7 表内恰 9 行（全文 12 处含图例与注记），README「9」正确；`manifest.rs` 规则 6/9（`min_host_version`、`unknown_capability`）已有实现与单测（L362/L744/L794），schema §7 与代码一致；protocol.md L61 上游命名注记经 PowerToys v0.101.2362.0 IDL 实抓核验（存在 `IFallbackHandler`、无 `IFallbackProvider`）。本轮改动 = 2×README + manifest-schema.md + protocol.md + implementation.md + CHANGELOG.md，纯文档零代码。

**当前状态（2026-09-10，v0.1.1 发版：版本 bump + 打包门禁 + tag 演练关账 M7 7.5）**：**首个正式版本迭代（v0.1.0 → v0.1.1）**。动机：tag `v0.1.0` 停在 `54b4b5d`，之后累积 4 个提交——文件搜索 v3.3 P0/P1/P1.5（more_commands 三动作 + 打开文件派发修复）、P2（everything-ipc 直连主通道 + es.exe 回落）、`file://` URL 三缺陷修复；`dist/` 产物（9/9 21:41）已滞后于代码，需一次正式发版把产物对齐并关账 M7 7.5。① **版本 bump**：5 个 crate（`dd-gui` / `dd-host` / `dd-ext` / `dd-run-cli` / `dd-ext-sample`）`0.1.0`→`0.1.1`（依赖均走 `path`、无版本约束，仅改 `[package] version`；`dd-protocol` 保持 `1.0.0` 不动）；`examples/extensions.d/com.ddrun.filesearch.json` 的 sidecar 清单版本同步 `0.1.1`（仅展示用途——已核验 `spec_manifest_capabilities_consistent` 只断言 capabilities 集合、清单未声明 `min_host_version` 故规则 6 不触发；唯一版本一致性断言是内置扩展 `builtin.rs` 的 `env!("CARGO_PKG_VERSION")`，随 bump 自动同步）。② **文档同步**：`CHANGELOG.md` 新增 `## [0.1.1] - 2026-09-10` 段头（保留空 `[Unreleased]`）；README 双语（2 处 + 2 处）与 implementation.md 顶部产物名 `dist/dd-run-0.1.0.exe`→`dist/dd-run-0.1.1.exe`。③ **打包**（`cargo clean -p dd-ext` → `bash tools/package.sh`，7m54s）：`dist/dd-run-0.1.1.exe` **10,854,912 B**；Python 按 `MZ`→`e_lfanew`→`PE\0\0` 校验**内嵌 PE = 6**（宿主 + 5 内置扩展，偏移 `0x0 / 0x5350c7 / 0x5da8d6 / 0x6510e7 / 0x6cdefb / 0x73f50b`）；`dist/extensions.d/` = `dd-ext-search.exe`（749,568 B，含 P2 IPC 代码）+ 清单（version 0.1.1、capabilities 含 `host/set_clipboard`）；本地稳定名 `dist/dd-run.exe` 同步覆盖为 0.1.1（前轮进程占用锁已解除）。④ **门禁**（windows-gnu，`APPDATA` 导出 + `CARGO_INCREMENTAL=0`）：`fmt --all --check` 0 diff、`clippy --workspace --all-targets -- -D warnings` 0 告警、`test --workspace`（CI 同口径 skip steam 机器相关用例）**324 passed / 0 failed**。⑤ **tag 演练**：`v0.1.1` push → `release.yml`（`tags: ['v*']`）→ `package.sh` → `Compress-Archive` 打 `dist/dd-run-0.1.1-portable.zip`（exe + `extensions.d/`）→ `action-gh-release` 附着 GitHub Release，关账 M7 7.5「tag 演练发版」。**执行结果（2026-09-10 21:19）**：commit `5668a37` → push `main` + tag `v0.1.1`；CI（run `34481293117`）✅ success；Release（run `34481300594`，6m42s）✅ success；GitHub Release **v0.1.1** 已发布（非 draft / 非 prerelease），附件 `dd-run-0.1.1-portable.zip` **5,610,327 B**（与 v0.1.0 并存）。至此 M7 7.5 仅剩**真机解压即用走查**（用户侧）。（本批改动集：5×Cargo.toml + Cargo.lock + CHANGELOG + 2×README + implementation.md + sidecar 清单。）

**当前状态（2026-09-10，方向 C 打磨批次：warm 空闲回收 + L5 修复 + README 口径修正，未 commit）**：**方向 C（性能/质量打磨）落地**。① **背景与决策**：上轮方向分析给出三项候选（M8 扩展生态 / v0.2 跨平台 / C 打磨），用户选 **C**；其中回收策略经用户拍板 = **全部回收 · 120s**。② **warm 进程空闲超时回收**：动因 = `LRU_WARM_CAPACITY`(8) 大于扩展总数（内置 5 + 官方 sidecar 1 = 6）→ **LRU 永不触发驱逐**、保活集行为上"只增不减"（稳态常驻全部扩展）。实现：`dd-host::cache::LruWarmSet` 每项记录「最后触达时刻」（`order: VecDeque<(String, Instant)>` + `access_at` 显式时刻版便于单测），新增只读 `idle_victims(_at)(ttl)` / `last_access`；`dd-gui` 新增 `warm_idle_reclaim()`（**复用既有 `evict_warm` 驱逐路径**：`close` + 回落 stub）。三条守卫：① 仅**面板隐藏**时回收（`self.visible` 为真直接返回，避免"看着就变慢"）；② 在途请求（进程已 take 出保活集）跳过，等归还后再判；③ 去掉 `refresh_health` 的 `exited.is_empty()` 早退，使回收独立于崩溃巡检（进程存活但长期未用）。隐藏期以 `WARM_IDLE_HIDDEN_TICK=15s` 调 `ctx.request_repaint_after` 驱动巡检（egui 隐藏窗口默认不产帧；保活集清空后不再请求，恢复静默）。③ **L5 销项**（§6.1 台账关闭）：根因精确定位为协议 §6.4 `get_command` **只查顶层命令**；修复 = `FallbackStore::contains_template` + 复热跳过校验（`invoke` 侧新增 `skip_lookup` 形参；`page` 侧把模板 id 过滤为 `None`，等价 GoToPage「无对应命令点击」语义）。**常规命令保持 §6.4 陈旧校验不变**。④ **README 双语跨平台口径修正**：`cross-platform (Windows / macOS / Linux)` 属**设计目标**而非现状（平台层全为 `#[cfg(not(windows))]` 占位、仅 Windows 可运行）→ 改为「核心与平台无关；**当前仅发行 Windows 版**，macOS / Linux 计划 v0.2+」，对比表表头 → 「平台无关设计」，Goals 首条加「计划中，当前仅 Windows」。**验证**：`fmt --all --check` 0 diff、`clippy --workspace --all-targets -- -D warnings` 0 告警、`test --workspace`（CI 同口径 skip steam）**330 passed / 0 failed**（较上轮 324 **+6**：dd-host cache 3 + dd-gui fallback 1 / health 2）；改动 9 文件（2×README + 6 dd-gui + 1 dd-host）。**真机验收待做**：① 空闲回收日志与内存（`Get-Process dd-run,dd-ext* | Select Id,ProcessName,@{n='PrivMB';e={[math]::Round($_.PrivateMemorySize64/1MB,2)}}`，对比空闲 120s 前后；日志形如 `[dd-gui] warm 空闲回收：<id>（空闲 Ns ≥ 120s…）`）；② 回收后按 Enter 走桩复热仍正常执行；③ **兜底模板直执行**——驱逐后点 `= 表达式` / `搜 xx` 等兜底项应正常出结果，而非「命令已失效」。

**当前状态（2026-09-10，M8 批次 8.1–8.4：扩展生态验证，未 commit）**：**"协议与语言无关"从声明变成实证**。用户决策（三选一均取推荐项）：仅 Python 单语言 / 示例做全表面 / 做 `--conformance`。① **8.1 指南** [`docs/extensions.md`](./extensions.md)：30 秒心智模型 → 10 分钟上手（目录 → 清单 → 程序 → 启动器 → 自检）→ 三条铁律（stdout 只出协议 / UTF-8 且无裸换行 / 通知不回复）→ **三个 Windows 陷阱**（文本模式的 `\n`→`\r\n`；stdout 默认编码可能是 cp936 而非 UTF-8——后者**不容忍**，附 `reconfigure(encoding="utf-8", newline="\n")` 与 `ensure_ascii` 两条修法；**解释器不在 PATH 上**——同日真机修复新增）→ 握手 → 各方法按「值得实现」排序 → `host/*` 反向请求（不必阻塞等应答）→ 自检 → **调试手册（8 条症状→原因对照表）** → 崩溃语义 → 超时预算。**只写规范里没有的东西**，与 `protocol.md`/`manifest-schema.md` 重复处一律改为引用。② **8.2 示例** [`examples/python-minimal/`](../examples/python-minimal/)：`dd_ext_pymin.py`（仅标准库，逐段注释对齐协议章节）+ `install.py`（**安装脚本：生成绝对路径清单**，推荐入口）+ `dd-ext-pymin.cmd`（备选启动器）+ README。覆盖 7 个 host→ext 方法、`items_changed` 通知、3 种 `host/*` 请求、`ShowToast`/`Dismiss`/`Confirm`（含确认后重发 `invoke` 的幂等两段式）。③ **8.3 自检器** `crates/dd-run-cli --conformance`：从 4 步扩到全表面 10 项（见 M8 章节表），逐项 `✓/⚠/✗`、任一 ✗ 退出码非 0；新增 `--invoke`（真实副作用默认跳过）与 `--ext-id`；新增单测 3 条。④ **8.4 实证**：`dd-run-cli --conformance --extensions-dir examples/python-minimal --invoke` → **10 项全 ✓（2386 ms）**。

**本批实测发现（都写进了指南，避免后人重踩）**：① **`.cmd` 可被 Rust `Command` 直接 spawn**（实测 27 ms）——原先只是 `resolve_executable` 里尝试 `.cmd`/`.bat` 的意图，本批首次实证；② 清单 `entry.args` **不支持** `${EXT_DIR}` 展开（`manifest-schema` §4 只覆盖 `command`/`cwd`/`icon`）→ 解释型扩展必须用启动器桥接，这是"清单无法表达解释器+脚本"的根因；③ 无扩展名文件与 `.cmd` 同名共存时，宿主 `is_file()` 会先命中无扩展名者 → Windows 上 spawn 失败（指南已标 ⚠️）；④ **自检器自身的判据反了**：`--conformance` 首版把「兜底一致性」写成 `fallback.is_empty() == has_fallback`（正确为 `!=`），对**完全合规**的示例报了红——已抽成纯函数 `fallback_is_consistent` 并补四象限单测消费掉这类反转；⑤ **自检器的 `invoke` 判据同样读错了层级**——读 `v["result"]["kind"]`（双层）而非内层 `kind`，与文档里错误的双层示例**互相印证** → 对不合规扩展判绿，直到真机才暴露（详见下方「真机反馈修复 2」）。

**验证**：`fmt --all --check` 0 diff、`clippy --workspace --all-targets -- -D warnings` 0 告警、`test --workspace`（CI 同口径 skip steam）全绿（**+3**：dd-run-cli 单测）。**改动集**：新增 `docs/extensions.md` + `examples/python-minimal/`（4 文件）+ `crates/dd-run-cli/src/main.rs`；同步 `CHANGELOG.md` / `implementation.md` / 2×README。协议 v1.0 **零改动**（本批只用既有方法与字段）。**真机验收 ✅（2026-09-10，用户确认）**：示例装入 `dist/extensions.d/` → 面板输入 `pymin` → 命令在 GUI 内**全部正常执行**（含此前报 `missing field kind` 的 invoke 类命令与兜底项）。

> **同日真机反馈已修（重要）**：用户按上述方式装上后，扩展管理**列出了 PyMin 但标为「暂时不可用」**、点「重试」无效。根因不是协议问题，而是**示例的启动器依赖 PATH**——`.cmd` 里的裸 `python` 在 GUI 的系统 PATH 下找不到（本机 `where python` **无输出**），而自检从**终端**启动时 PATH 恰好有解释器，于是**「自检全绿」掩盖了「GUI 起不来」**。修复 = 改为 `install.py` 生成**绝对路径清单**（宿主直接 spawn、不经 shell，**零 PATH 依赖**），静态清单移除、`.cmd` 降级为备选；指南同步新增**陷阱 3「解释器不在 PATH 上」**并明写「自检全绿 ≠ GUI 能启动」。**验证**：PATH 摘到只剩 `System32` 后 10 项全 ✓（**902 ms**）。详见 M8 章节「真机反馈修复」段。

> **真机反馈修复 2（`invoke` 响应形状：同一错误被文档/类型/单测/自检器四处固化）**：PyMin 装好后 `当前时间` / `添加便签` / `清空便签` / `打开文档` 四条命令报 `命令执行失败：非法 JSON-RPC 信封：missing field ``kind```（`便签列表` 为嵌套页命令、不走 `invoke`，不受影响）。抓包（模拟宿主的探针逐条 `invoke` 打印原始行）显示扩展实发 `{"jsonrpc":"2.0","id":N,"result":{"result":{"kind":...}}}`——**多包了一层**。§6.5 规定信封的 `result` **就是** `CommandResult` 本体，`dd-host::invoke()` 也正是 `from_value::<CommandResult>(内层 result)`；但**四处**都写成双层：`protocol.md` §6.5/§14 示例、`dd-protocol::InvokeResult`（`result` 域内再嵌一层）、`consistency.rs` §6.5 断言、M8 新写的 `extensions.md` 示例。M2 只修了宿主，其余未同步 → 照文档写的示例必踩。**更严重**：`--conformance` 的判据读 `v["result"]["kind"]`，与错误形状**恰好吻合** → 判绿。修复：示例改 `reply(mid, result)`；协议示例改单层并加告诫；**删除虚构的 `InvokeResult`**，单测改断言 `Resp<CommandResult>`；自检判据改判内层 `kind` 并抽成纯函数 `command_result_kind`（**识别并指名「多包了一层」**）；指南 §5.4 + §8 症状表补该条。**验证**：抓包复核四条 `invoke` 均单层；`--conformance --invoke` **10 项全 ✓**；**反向验证**——把示例改回双层后自检器正确报 `✗ result 被多包了一层`（修复前对同一份扩展判绿）；新增单测 1 条。**教训**：形状类契约一旦「文档 + 类型 + 单测 + 工具」四方同错则无人报红——**必须让至少一个环节面对真实线缆**（本次为抓包探针）。协议 v1.0 零改动。

...
| M6 日常打磨（2026-09-05 立项，方向经用户确认） | ✅ 代码全部完成（批次 6.1–6.5，commits `a007656`…`2e5b3da`）；剩余为真机验收项（6.3 三项设置功能、L8 C1–C3、L9 IME、v4.17a 亚克力/拖拽修复）——见 M7 立行下「M6 集中回归清单」 | 拼音搜索（L4）、冷启动字体后台加载（L10）、设置三子页功能化（热键重绑/开机自启/扩展管理）、L2/L3/L7 销项、三子页 UI 优化（v4.8/v4.9） |
| M7 发布工程（2026-09-08 立项） | 🚧 7.1 CI ✅（commit `a789906`，CI 首跑绿灯 4m23s）/ 7.2 256px 图标 ✅；7.3 安装器 / 7.4 文件搜索分发定案 / 7.5 发布流程 待做 | 见下方 M7 章节 |
...
按 2026-09-05 既定路线（日常好用 → 发布工程 → 扩展生态验证）进入 M7：把项目从「能自用」推向「能分发」。M6 剩余真机验收项不阻塞本里程碑开发，以「M6 集中回归清单」（见 §7 当前状态 2026-09-08 条目）集中关账。

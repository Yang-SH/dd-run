...
| 7.3 安装器 | Inno Setup 脚本（单文件 `dd-run.exe` + `extensions.d/` sidecar → 安装目录、开始菜单快捷方式、可选开机自启复用现有注册表 Run 键、卸载清理缓存目录可选保留用户配置） | 真机安装/升级/卸载走查 | 待做 |
| 7.4 文件搜索分发定案 | 决策：`dd-ext-search` 保持 sidecar（安装器携带 `extensions.d/`）或纳入内嵌物化机制（`build.rs` EMBED_EXES 表扩第 6 个成员） | 决策记档 + 实施（或明确延后理由） | 待做 |
| 7.5 发布流程 | tag → `package.sh` 产物 + GitHub Release 附着（dist 单文件 + sidecar 目录 zip） | 一次演练发版全流程 | 待做 |

已做取舍记档（7.1）：CI 测试显式跳过 `apps::steam_installed_shown_uninstalled_filtered_root_lnk_shown`——该测试硬编码期望本机已装应用「Flowframes」，机器相关（2026-09-07 记录），CI 环境无法满足；后续批次改为机器无关夹具后移除跳过。已做取舍记档（7.2）：256 档沿用 BMP 条目（与既有五档同格式，无新增消费方差异；PNG 压缩条目仅省体积，`.ico` 总尺寸非当前瓶颈）。

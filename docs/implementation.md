...
| 6.3 设置占位落地 | 全局热键自定义（重注册 + 冲突提示）/ 开机自启（注册表 Run 键）/ 扩展管理页（清单列表 + 启停） | 各开关即时生效 + 持久化 + 重启保留；热键冲突有提示 | 待做 |
| 6.4 遗留清扫 | L3 顶层 items_changed 重聚合接线 / L2 熔断手动重试入口 / L7 apps.rs clippy / L1 启动一帧闪屏 | 各遗留项台账销项 | 待做 |

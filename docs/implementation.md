...
| M7 发布工程（2026-09-08 立项） | 🚧 7.1 CI / 7.2 256px 图标 代码完成（CI 首次云端运行待验证）；7.3 安装器 / 7.4 文件搜索分发定案 / 7.5 发布流程 待做 | 见下方 M7 章节 |
